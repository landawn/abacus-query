...
- Build: cdd4e8a36542614aa2ede7c93ca0e333f79191f9
- Java: 17
...
- **Examples:**
  - ```java
    ...
    ```
##### distinctOn(...) -> This
- **Signature:** `public This distinctOn(final String expressions)`
- **Summary:** Adds PostgreSQL-style DISTINCT ON (expressions) to the SELECT statement.
- **Contract:**
...
- **Examples:**
  - ```java
    ...
    ```
##### distinctRow(...) -> This
- **Signature:** `public This distinctRow()`
- **Summary:** Adds the MySQL-style DISTINCTROW modifier to the SELECT statement.
- **Contract:**
...
- **Examples:**
  - ```java
    ...
    ```
- **Signature:** `public This from(final This sqlBuilder, final String alias)`
- **Summary:** Uses a complete SELECT built by another builder as a derived table in this query's FROM clause.
- **Contract:**
...
- **Contract:**
  - An unqualified property name in a later clause is therefore resolved against the FROM table, and when the FROM table was given as a plain string (which carries no entity mapping) such a name is emitted verbatim and unqualified - ambiguous if both tables define it.
- **Parameters:**
...
- **Contract:**
  - When there are multiple elements, each expression is enclosed in parentheses before the fragments are combined with AND, preserving the precedence of any OR within an expression.
- **Parameters:**
...
- **Returns:** this SqlBuilder instance for method chaining
- **Throws:**
  - `java.lang.IllegalArgumentException` — if propOrColumnNames is null or empty, contains a null, empty, or blank element, or renders a qualified column name (a dot outside a quoted identifier), which USING (...) does not accept
  - `java.lang.IllegalStateException` — if there is no immediately preceding JOIN that accepts an ON/USING connector
- **Examples:**
  - ```java
    String sql = PSC.select("*")
...
- **Throws:**
  - `java.lang.IllegalArgumentException` — if expr is null, empty, or blank, or, on a SQL Server dialect, contains a temporary-table identifier (#name) together with a SQL comment token
  - `java.lang.IllegalStateException` — if WHERE has already been set on this builder, or if a preceding qualified JOIN has not been completed with on(...)/using(...)
- **Examples:**
  - ```java
    String sql = PSC.select("*")
                    .from("users")
...
- **Throws:**
  ...
  - `java.lang.IllegalStateException` — if GROUP BY has already been set on this builder, or if a preceding qualified JOIN has not been completed with on(...)/using(...)
- **Examples:**
  - ```java
...
- **Throws:**
  ...
  - `java.lang.IllegalStateException` — if this builder is closed, if GROUP BY has already been set on this builder, or if a preceding qualified JOIN has not been completed with on(...)/using(...)
- **Examples:**
  - ```java
    String sql = PSC.select("category", "COUNT(*)")
                    .from("products")
...
- **Throws:**
  - `java.lang.IllegalArgumentException` — if expr is null, empty, or blank, or, on a SQL Server dialect, contains a temporary-table identifier (#name) together with a SQL comment token
  - `java.lang.IllegalStateException` — if HAVING has already been set on this builder, or if a preceding qualified JOIN has not been completed with on(...)/using(...)
- **Examples:**
  - ```java
...
- **Throws:**
  - `java.lang.IllegalArgumentException` — if propOrColumnName is null, empty, or blank
  - `java.lang.IllegalStateException` — if ORDER BY has already been set on this builder, or if a preceding qualified JOIN has not been completed with on(...)/using(...)
- **Examples:**
  - ```java
    String sql = PSC.select("*")
                    .from("users")
...
- **Throws:**
  - `java.lang.IllegalArgumentException` — if expr is null, empty, or blank, if expr contains a SQL comment token, or if direction is null
  - `java.lang.IllegalStateException` — if this builder is closed, if ORDER BY has already been set on this builder, or if a preceding qualified JOIN has not been completed with on(...)/using(...)
- **Examples:**
  - ```java
    String sql = PSC.select("*")
                    .from("users")
...
- **Throws:**
  ...
  - `java.lang.IllegalStateException` — if this builder has already been closed by #build(), if LIMIT or OFFSET has already been set on this builder, on a FETCH-style dialect if FETCH FIRST/FETCH NEXT has already been set, for SQL Server if ORDER BY has not been set, on UPDATE/DELETE statements (the OFFSET portion is not valid there), or if a preceding qualified JOIN has not been completed with on(...)/using(...)
- **Examples:**
  - ```java
    String sql = PSC.select("*")
                    .from("users")
...
- **Throws:**
  ...
  - `java.lang.IllegalStateException` — if this builder is closed, if OFFSET has already been set on this builder, for SQL Server if ORDER BY has not been set, if a preceding qualified JOIN has not been completed with on(...)/using(...), on UPDATE/DELETE statements, or after FOR UPDATE has been emitted
- **Examples:**
  - ```java
    String sql = PSC.select("*")
                    .from("users")
...
- **Throws:**
  - `java.lang.IllegalArgumentException` — if count is negative
  - `java.lang.IllegalStateException` — if this builder is closed, if LIMIT, FETCH NEXT, or FETCH FIRST has already been set, if a prior offset used limit-style OFFSET n rather than OFFSET n ROWS, for SQL Server if ORDER BY has not been set, if a preceding qualified JOIN has not been completed with on(...)/using(...), on UPDATE/DELETE statements, or after FOR UPDATE has been emitted
- **Examples:**
  - ```java
    String sql = PSC.select("*")
                    .from("users")
...
- **Throws:**
  - `java.lang.IllegalArgumentException` — if count is negative
  - `java.lang.IllegalStateException` — if this builder is closed, if LIMIT, FETCH FIRST, or FETCH NEXT has already been set, if a prior offset used limit-style OFFSET n rather than OFFSET n ROWS, for SQL Server if ORDER BY has not been set, if a preceding qualified JOIN has not been completed with on(...)/using(...), on UPDATE/DELETE statements, or after FOR UPDATE has been emitted
- **Examples:**
  - ```java
    String sql = PSC.select("*")
                    .from("users")
...
- **Contract:**
  - When the statement built so far is nonempty, a separating space is inserted before expr if the statement does not already end with a space and expr does not already begin with one.
  - Caller-supplied edge whitespace is otherwise preserved; if the statement ends with a space and expr begins with one, both spaces remain.
- **Parameters:**
...
- **Throws:**
  ...
  - `java.lang.IllegalStateException` — if this builder or sqlBuilder is closed; if this builder is not building a SELECT query, its current SELECT segment has not been completed by from(...), or ORDER BY, pagination, or FOR UPDATE has already been added
- **Examples:**
  - ```java
...
- **Contract:**
  - The query text is appended without adding branch isolation; callers must supply any grouping required by their database for branch-local clauses or nested set operations.
- **Parameters:**
  - `query` (`String`) — the complete SELECT sub-query to union (lexically SELECT-only; a syntactic check, not a read-only guarantee)
- **Returns:** this SqlBuilder instance for method chaining
- **Throws:**
...
- **Examples:**
  - ```java
    ...
    ```
##### unionSelect(...) -> This
- **Signature:** `public This unionSelect(final Collection<String> propOrColumnNames)`
...
- **Contract:**
  - The query text is appended without adding branch isolation; callers must supply any grouping required by their database for branch-local clauses or nested set operations.
- **Parameters:**
  - `query` (`String`) — the complete SELECT sub-query to union all (lexically SELECT-only; a syntactic check, not a read-only guarantee)
- **Returns:** this SqlBuilder instance for method chaining
- **Throws:**
  - `java.lang.IllegalArgumentException` — if query is null, empty, blank, or is not a complete, lexically SELECT-only SELECT sub-query
  - `java.lang.IllegalStateException` — if this builder is closed, is not building a SELECT query, the current SELECT segment has not been completed by from(...), or ORDER BY, pagination, or FOR UPDATE has already been added
- **Examples:**
  - ```java
    ...
    ```
##### unionAllSelect(...) -> This
- **Signature:** `public This unionAllSelect(final Collection<String> propOrColumnNames)`
...
- **Contract:**
  - The query text is appended without adding branch isolation; callers must supply any grouping required by their database for branch-local clauses or nested set operations.
- **Parameters:**
  - `query` (`String`) — the complete SELECT sub-query to intersect (lexically SELECT-only; a syntactic check, not a read-only guarantee)
- **Returns:** this SqlBuilder instance for method chaining
- **Throws:**
  - `java.lang.IllegalArgumentException` — if query is null, empty, blank, or is not a complete, lexically SELECT-only SELECT sub-query
  - `java.lang.IllegalStateException` — if this builder is closed, is not building a SELECT query, the current SELECT segment has not been completed by from(...), or ORDER BY, pagination, or FOR UPDATE has already been added
- **Examples:**
  - ```java
    ...
    ```
##### intersectSelect(...) -> This
- **Signature:** `public This intersectSelect(final Collection<String> propOrColumnNames)`
...
- **Contract:**
  - The query text is appended without adding branch isolation; callers must supply any grouping required by their database for branch-local clauses or nested set operations.
- **Parameters:**
  - `query` (`String`) — the complete SELECT sub-query to except (lexically SELECT-only; a syntactic check, not a read-only guarantee)
- **Returns:** this SqlBuilder instance for method chaining
- **Throws:**
  - `java.lang.IllegalArgumentException` — if query is null, empty, blank, or is not a complete, lexically SELECT-only SELECT sub-query
  - `java.lang.IllegalStateException` — if this builder is closed, is not building a SELECT query, the current SELECT segment has not been completed by from(...), or ORDER BY, pagination, or FOR UPDATE has already been added
- **Examples:**
  - ```java
    ...
    ```
##### exceptSelect(...) -> This
- **Signature:** `public This exceptSelect(final Collection<String> propOrColumnNames)`
...
- **Contract:**
  - The query text is appended without adding branch isolation; callers must supply any grouping required by their database for branch-local clauses or nested set operations.
- **Parameters:**
  - `query` (`String`) — the complete SELECT sub-query to subtract with MINUS (lexically SELECT-only; a syntactic check, not a read-only guarantee)
- **Returns:** this SqlBuilder instance for method chaining
- **Throws:**
  - `java.lang.IllegalArgumentException` — if query is null, empty, blank, or is not a complete, lexically SELECT-only SELECT sub-query
  - `java.lang.IllegalStateException` — if this builder is closed, is not building a SELECT query, the current SELECT segment has not been completed by from(...), or ORDER BY, pagination, or FOR UPDATE has already been added
- **Examples:**
  - ```java
    ...
    ```
##### minusSelect(...) -> This
- **Signature:** `public This minusSelect(final Collection<String> propOrColumnNames)`
...
- **Examples:**
  - ```java
    ...
    ```
- **Signature:** `public This set(final String propOrColumnName, final Object value)`
- **Summary:** Sets one UPDATE column to a supplied value.
- **Parameters:**
  - `propOrColumnName` (`String`) — the property or column to assign
...
- **Examples:**
  - ```java
    ...
    ```
- **Signature:** `public This set(final String propOrColumnName1, final Object value1, final String propOrColumnName2, final Object value2)`
- **Summary:** Sets two UPDATE columns to supplied values in argument order.
- **Parameters:**
  - `propOrColumnName1` (`String`) — the first property or column to assign
  - `value1` (`Object`) — the first value to render, or a SqlExpression to embed
  - `propOrColumnName2` (`String`) — the second property or column to assign
...
- **Examples:**
  - ```java
    ...
    ```
- **Signature:** `public This set(final String propOrColumnName1, final Object value1, final String propOrColumnName2, final Object value2, final String propOrColumnName3, final Object value3)`
- **Summary:** Sets three UPDATE columns to supplied values in argument order.
- **Parameters:**
  - `propOrColumnName1` (`String`) — the first property or column to assign
  - `value1` (`Object`) — the first value to render, or a SqlExpression to embed
  - `propOrColumnName2` (`String`) — the second property or column to assign
  - `value2` (`Object`) — the second value to render, or a SqlExpression to embed
  - `propOrColumnName3` (`String`) — the third property or column to assign
...
- **Returns:** this SqlBuilder instance for method chaining
- **Throws:**
  - `java.lang.IllegalArgumentException` — if entity is null, if a bean has no updatable property, or if entity is a Collection or array (use #set(Collection) for column lists)
  - `java.lang.IllegalStateException` — if this builder is closed, does not represent an UPDATE, or a post-SET clause such as WHERE has already been emitted
- **Examples:**
  - ```java
...
- **Returns:** this SqlBuilder instance for method chaining
- **Throws:**
  - `java.lang.IllegalArgumentException` — if entity is null, if entity is a Collection or array (use #set(Collection) for column lists), or if a bean entity has no updatable property, or a Map entity has no entry, remaining after exclusions are applied
  - `java.lang.IllegalStateException` — if this builder is closed, does not represent an UPDATE, or a post-SET clause such as WHERE has already been emitted
- **Examples:**
  - ```java
...
- **Contract:**
  - Entity metadata selection and SET rendering are atomic: if parameter rendering fails, this builder is restored to its state before this call.
- **Parameters:**
  - `entityClass` (`Class<?>`) — the entity class to get properties from
...
- **Contract:**
  - Once final rendering begins, the builder is closed even if rendering fails.
- **Parameters:**
  - (none)
...
- **Throws:**
  ...
  - `java.lang.IllegalStateException` — if this builder is closed
- **Examples:**
  - ```java
...
#### Public Constructors
- **Signature:** `public SP`
- **Summary:** Creates an immutable SQL/parameter pair.
- **Parameters:**
  - `query` (`String`)
  - `parameters` (`ImmutableList<Object>`)
- **Throws:**
  - `java.lang.IllegalArgumentException` — if query or parameters is null
...
#### Public Fields
- **Declaration:** `public static final Dsl PSB`
  - **Summary:** Parameterized-SQL DSL (? placeholders) that leaves property/column names unchanged.
  - **Nullability:** unspecified
- **Declaration:** `public static final Dsl PSC`
  - **Summary:** Parameterized-SQL DSL (? placeholders) with snake_case naming.
  - **Nullability:** unspecified
- **Declaration:** `public static final Dsl PAC`
  - **Summary:** Parameterized-SQL DSL (? placeholders) with SCREAMING_SNAKE_CASE (UPPER_CASE_WITH_UNDERSCORE) naming.
  - **Nullability:** unspecified
- **Declaration:** `public static final Dsl PLC`
  - **Summary:** Parameterized-SQL DSL (? placeholders) with lowerCamelCase naming.
  - **Nullability:** unspecified
- **Declaration:** `public static final Dsl NSB`
  - **Summary:** Named-SQL DSL (:name placeholders) that leaves property/column names unchanged.
  - **Nullability:** unspecified
- **Declaration:** `public static final Dsl NSC`
  - **Summary:** Named-SQL DSL (:name placeholders) with snake_case naming.
  - **Nullability:** unspecified
- **Declaration:** `public static final Dsl NAC`
  - **Summary:** Named-SQL DSL (:name placeholders) with SCREAMING_SNAKE_CASE (UPPER_CASE_WITH_UNDERSCORE) naming.
  - **Nullability:** unspecified
- **Declaration:** `public static final Dsl NLC`
  - **Summary:** Named-SQL DSL (:name placeholders) with lowerCamelCase naming.
  - **Nullability:** unspecified
- **Declaration:** `public static final Dsl SCSB`
  - **Summary:** Raw-SQL DSL with snake_case naming; values are inlined as SQL literals rather than parameterized.
  - **Deprecated:** #PSC or #NSC is preferred for better security and performance. Un-parameterized SQL is vulnerable to SQL injection attacks.
  - **Nullability:** unspecified
- **Declaration:** `public static final Dsl ACSB`
  - **Summary:** Raw-SQL DSL with SCREAMING_SNAKE_CASE (UPPER_CASE_WITH_UNDERSCORE) naming; values are inlined as SQL literals rather than parameterized.
  - **Deprecated:** #PAC or #NAC is preferred for better security and performance. Un-parameterized SQL is vulnerable to SQL injection attacks.
  - **Nullability:** unspecified
- **Declaration:** `public static final Dsl LCSB`
  - **Summary:** Raw-SQL DSL with lowerCamelCase naming; values are inlined as SQL literals rather than parameterized.
  - **Deprecated:** #PLC or #NLC is preferred for better security and performance. Un-parameterized SQL is vulnerable to SQL injection attacks.
  - **Nullability:** unspecified
- **Declaration:** `public static final Dsl MSB`
  - **Summary:** iBATIS/MyBatis-SQL DSL (#{name} placeholders) that leaves property/column names unchanged.
  - **Deprecated:** Use #NSB instead. Note: Switching from MSB to NSB changes the parameter style from iBATIS (#{param}) to named (:param).
  - **Nullability:** unspecified
- **Declaration:** `public static final Dsl MSC`
  - **Summary:** iBATIS/MyBatis-SQL DSL (#{name} placeholders) with snake_case naming.
  - **Deprecated:** Use #NSC instead. Note: Switching from MSC to NSC changes the parameter style from iBATIS (#{param}) to named (:param).
  - **Nullability:** unspecified
- **Declaration:** `public static final Dsl MAC`
  - **Summary:** iBATIS/MyBatis-SQL DSL (#{name} placeholders) with SCREAMING_SNAKE_CASE (UPPER_CASE_WITH_UNDERSCORE) naming.
  - **Deprecated:** Use #NAC instead. Note: Switching from MAC to NAC changes the parameter style from iBATIS (#{param}) to named (:param).
  - **Nullability:** unspecified
- **Declaration:** `public static final Dsl MLC`
  - **Summary:** iBATIS/MyBatis-SQL DSL (#{name} placeholders) with lowerCamelCase naming.
  - **Deprecated:** Use #NLC instead. Note: Switching from MLC to NLC changes the parameter style from iBATIS (#{param}) to named (:param).
  - **Nullability:** unspecified
...
- **Returns:** a new SqlBuilder instance configured for INSERT operation
- **Throws:**
  - `java.lang.IllegalArgumentException` — if entity is null; if a String entity is blank; if a Map entity is empty or has a non-String or blank key; or if a bean has no insertable value left after its null values and default-valued ID properties are skipped (a non-ID primitive still holding its default, such as 0, is kept)
- **Examples:**
  - ```java
...
- **Returns:** a new SqlBuilder instance configured for INSERT operation
- **Throws:**
  - `java.lang.IllegalArgumentException` — if entity is null; if a String entity is blank; if a Map entity is empty, has a non-String or blank key, or has no entries left after exclusions are applied; or if a bean has no insertable value left after exclusions are applied and its null values and default-valued ID properties are skipped (a non-ID primitive still holding its default, such as 0, is kept)
- **Examples:**
  - ```java
    Account account = new Account();
    account.setFirstName("John");
...
- **Contract:**
  - Property names in WHERE conditions are rendered according to this DSL's naming policy, even when no entity class is associated.
- **Parameters:**
...
- **Contract:**
  ...
  - A comma-separated list is not split, and an alias must be introduced with AS: under a naming policy that changes the text (for example snake_case), an implicit alias such as "firstName fn" is treated as part of the expression and the whole text is aliased as a single column (rendering first_name fn AS "firstName fn"), a comma list such as "firstName, lastName" is aliased as one column, and identifiers after the first top-level AS are left unconverted.
- **Parameters:**
...
- **Deprecated:** hard to read at the call site (positional arguments) and limited to exactly two entities. Build a Selection per table (e.g. with Selection.builder(Entity.class)) and pass them to #select(List), which is self-documenting and supports any number of tables.
- **Parameters:**
...
- **Returns:** a new SqlBuilder instance configured for SELECT operation
- **Throws:**
  - `java.lang.IllegalArgumentException` — if either entity class is null, or if the two selections together resolve to no selectable property after exclusions are applied
- **Examples:**
  - ```java
...
- **Returns:** a new SqlBuilder instance configured for SELECT operation
- **Throws:**
  - `java.lang.IllegalArgumentException` — if selection is null or resolves to no selectable property, or carries a blank, quoted, or comment-bearing table or class alias
- **Examples:**
  - ```java
...
- **Returns:** a new SqlBuilder instance configured for SELECT operation
- **Throws:**
  - `java.lang.IllegalArgumentException` — if selections is null or empty, contains invalid data (including a blank, quoted, or comment-bearing table or class alias), or resolves to no properties in total
- **Examples:**
  - ```java
...
- **Deprecated:** hard to read at the call site (positional arguments) and limited to exactly two entities. Build a Selection per table (e.g. with Selection.builder(Entity.class)) and pass them to #selectFrom(List), which is self-documenting and supports any number of tables.
- **Parameters:**
  - `entityClassA` (`Class<?>`) — first entity class
  - `tableAliasA` (`String`) — table alias for first entity
...
- **Returns:** a new SqlBuilder instance with SELECT and FROM configured
- **Throws:**
  - `java.lang.IllegalArgumentException` — if either entity class is null, if the two selections together resolve to no selectable property after exclusions are applied, or if the generated FROM clause is blank
- **Examples:**
  - ```java
...
- **Returns:** a new SqlBuilder instance with SELECT and FROM configured
- **Throws:**
  - `java.lang.IllegalArgumentException` — if selection is null, resolves to no selectable property, carries a blank, quoted, or comment-bearing table or class alias, or produces a blank generated FROM clause
- **Examples:**
  - ```java
...
- **Returns:** a new SqlBuilder instance with SELECT and FROM configured
- **Throws:**
  - `java.lang.IllegalArgumentException` — if selections is null or empty, contains invalid data (including a blank, quoted, or comment-bearing table or class alias), produces a blank generated FROM clause, or resolves to no properties in total
- **Examples:**
  - ```java
    List<Selection> selections = Arrays.asList(
        Selection.builder(Account.class).tableAlias("a").classAlias("account").build(),
    ...
    String sql = PSC.selectFrom(selections)
                    ...
                    .build().query();
    ```
##### selectCountFrom(...) -> SqlBuilder
- **Signature:** `public SqlBuilder selectCountFrom(final String tableName)`
...
- **Examples:**
  - ```java
    ...
    // Output: SELECT count(*) FROM account WHERE status = ?
    ```
- **Signature:** `public SqlBuilder selectCountFrom(final Class<?> entityClass)`
...
- **Throws:**
  - `java.lang.IllegalArgumentException` — if count is negative
  - `java.lang.IllegalStateException` — if a limit count was already specified, SQL:2008 pagination was selected, or this builder has already been closed by a prior call to #build()
- **Examples:**
  - ```java
    Builder builder = DynamicQuery.builder();
    builder.select().append("*");
...
- **Contract:**
  ...
  - A zero offset still counts as a specified plain offset, so a later #offset(int) call throws IllegalStateException; use #limit(int) when the offset should remain open.
- **Parameters:**
  - `count` (`int`) — the maximum number of rows to return (must not be negative)
...
- **Throws:**
  - `java.lang.IllegalArgumentException` — if count or offset is negative
  - `java.lang.IllegalStateException` — if a limit or plain offset was already specified, SQL:2008 pagination was selected, or this builder has already been closed by a prior call to #build()
- **Examples:**
  - ```java
    Builder builder = DynamicQuery.builder();
    builder.select().append("*");
...
- **Contract:**
  - When offset is 0, the offset remains semantically specified but is omitted from the generated SQL.
...
- **Throws:**
  - `java.lang.IllegalArgumentException` — if offset is negative
  - `java.lang.IllegalStateException` — if a plain offset was already specified (by this method or by #limit(int, int), including with a zero offset), SQL:2008 pagination was selected, or this builder has already been closed by a prior call to #build()
- **Examples:**
  - ```java
...
- **Contract:**
  - An explicitly supplied zero is rendered as OFFSET 0 ROWS; this is useful when the target dialect requires an offset before FETCH NEXT.
- **Parameters:**
  - `offset` (`int`) — the number of rows to skip (must not be negative)
- **Returns:** this builder instance for method chaining
- **Throws:**
  - `java.lang.IllegalArgumentException` — if offset is negative
  - `java.lang.IllegalStateException` — if an OFFSET ... ROWS value was already specified, LIMIT-style pagination was selected, or this builder has already been closed by a prior call to #build()
- **Examples:**
  - ```java
...
- **Throws:**
  - `java.lang.IllegalArgumentException` — if count is negative
  - `java.lang.IllegalStateException` — if a fetch count was already specified, LIMIT-style pagination was selected, or this builder has already been closed by a prior call to #build()
- **Examples:**
  - ```java
...
- **Contract:**
  - When b is true this behaves exactly like #append(String) (a single separating space is inserted only when needed, then the text is emitted unchanged after non-blank validation, without escaping or interpretation); when b is false the builder is left unchanged and textToAppend is not inspected.
- **Parameters:**
...
- **Contract:**
  - Always appends something, choosing between the two options; the chosen text is emitted exactly as #append(String) would emit it (a single separating space is inserted only when needed, without SQL parsing, escaping, or interpretation).
  - Both alternatives must be non-blank, including the alternative that is not selected.
- **Parameters:**
  - `b` (`boolean`) — the condition to check
...
- **Throws:**
  - `java.lang.IllegalStateException` — if this clause builder has already been closed by build()
...
- **Returns:** this FromClause instance for method chaining
- **Throws:**
  - `java.lang.IllegalStateException` — if this clause builder has already been closed by build(), or if the FROM clause has not been initialized by a prior call that actually appended a table (e.g. append(...), appendIf(...) with a true condition, or appendIfOrElse(...))
...
- **Summary:** Adds a JOIN clause (implicit INNER JOIN) with the specified table and no separate connector step.
- **Contract:**
  - Any required ON/USING connector must be included in joinExpr itself; this overload may also be used for dialect-specific conditionless joins.
- **Parameters:**
  - `joinExpr` (`String`) — the table or entity to join (can include alias; must not be null, empty, or blank)
- **Returns:** this FromClause instance for method chaining
- **Throws:**
...
- **Summary:** Adds an INNER JOIN clause with the specified table and no separate connector step.
- **Contract:**
  - Any required ON/USING connector must be included in joinExpr itself; this overload may also be used for dialect-specific conditionless joins.
- **Parameters:**
  - `joinExpr` (`String`) — the table or entity to join (can include alias; must not be null, empty, or blank)
...
- **Summary:** Adds a LEFT JOIN clause with the specified table and no separate connector step.
- **Contract:**
  - Any required ON/USING connector must be included in joinExpr itself; this overload may also be used for dialect-specific conditionless joins.
- **Parameters:**
  - `joinExpr` (`String`) — the table or entity to join (can include alias; must not be null, empty, or blank)
...
- **Summary:** Adds a RIGHT JOIN clause with the specified table and no separate connector step.
- **Contract:**
  - Any required ON/USING connector must be included in joinExpr itself; this overload may also be used for dialect-specific conditionless joins.
- **Parameters:**
  - `joinExpr` (`String`) — the table or entity to join (can include alias; must not be null, empty, or blank)
...
- **Summary:** Adds a FULL JOIN clause with the specified table and no separate connector step.
- **Contract:**
  - Any required ON/USING connector must be included in joinExpr itself; this overload may also be used for dialect-specific conditionless joins.
- **Parameters:**
  - `joinExpr` (`String`) — the table or entity to join (can include alias; must not be null, empty, or blank)
...
- **Returns:** this WhereClause instance for method chaining
- **Throws:**
  - `java.lang.IllegalStateException` — if this clause builder has already been closed by build(), or if the WHERE clause has not been initialized by a prior call that actually appended a condition (e.g. append(...), and(...), or or(...))
...
- **Returns:** this HavingClause instance for method chaining
- **Throws:**
  - `java.lang.IllegalStateException` — if this clause builder has already been closed by build(), or if the HAVING clause has not been initialized by a prior call that actually appended a condition (e.g. append(...), and(...), or or(...))
  - `java.lang.IllegalArgumentException` — if placeholderCount is negative
- **Examples:**
  - ```java
...
#### Public Fields
- **Declaration:** `public static final SqlExpression QME`
  - **Summary:** A SQL expression representing a question mark literal (?) for use in parameterized SQL queries.
  - **Nullability:** unspecified
...
- **Deprecated:** dangerous; could silently bypass all filtering, returning all rows. Avoid using this method; restructure the conditional logic instead.
- **Parameters:**
  - (none)
...
- **Deprecated:** dangerous; could silently return zero rows. Avoid using this method; restructure the conditional logic instead.
- **Parameters:**
  - (none)
...
- **Throws:**
  - `java.lang.IllegalArgumentException` — if condition is null, or is non-composable — a Criteria, a clause (for example WHERE, HAVING, or ORDER BY), an ON/USING connector, an ANY/ALL/SOME quantified-subquery operand, a standalone SubQuery, or a blank SqlExpression
- **Examples:**
  - ```java
...
- **Parameters:**
  - `propName` (`String`) — the property/column name (must not be null, empty, or blank)
  - `operator` (`Operator`) — the binary comparison operator to use (must not be null; membership, IS/IS NOT, and structural operators are rejected)
- **Returns:** a Binary condition with a ? placeholder value
- **Throws:**
  - `java.lang.IllegalArgumentException` — if propName is null, empty, or blank, or if operator is IN, NOT_IN, IS, IS_NOT, or is not a valid binary comparison operator
  - `java.lang.NullPointerException` — if operator is null
- **Examples:**
  - ```java
...
- **Throws:**
  - `java.lang.IllegalArgumentException` — if props is null or empty, or any property name key is null, empty, or blank; if a condition-valued entry is invalid as a scalar operand, including a structured SubQuery whose known, non-wildcard projection contains more than one column
- **Examples:**
  - ```java
    Map<String, Object> props = new LinkedHashMap<>();
    props.put("name", "John");
...
- **Summary:** Creates an OR condition from an entity object using its selectable top-level properties.
- **Parameters:**
  - `entity` (`Object`) — the entity object whose properties will be used
- **Returns:** an Or condition
- **Throws:**
  - `java.lang.IllegalArgumentException` — if entity is null, is a map (use #anyEqual(Map)), or its class declares no selectable property; if a condition-valued entry is invalid as a scalar operand, including a structured SubQuery whose known, non-wildcard projection contains more than one column
- **Examples:**
  - ```java
...
- **Returns:** an Or condition
- **Throws:**
  - `java.lang.IllegalArgumentException` — if entity is null or is a map, or if includedPropNames is null, empty, or contains a null, empty, blank, or unreadable name; if a condition-valued entry is invalid as a scalar operand, including a structured SubQuery whose known, non-wildcard projection contains more than one column
- **Examples:**
  - ```java
...
- **Returns:** an Or condition
- **Throws:**
  - `java.lang.IllegalArgumentException` — if any property name is null, empty, or blank; if a condition-valued entry is invalid as a scalar operand, including a structured SubQuery whose known, non-wildcard projection contains more than one column
- **Examples:**
  - ```java
...
- **Summary:** Creates an AND condition from an entity object using its selectable top-level properties.
- **Parameters:**
  - `entity` (`Object`) — the entity object whose properties will be used
- **Returns:** an And condition
- **Throws:**
  - `java.lang.IllegalArgumentException` — if entity is null, is a map (use #allEqual(Map)), or its class declares no selectable property; if a condition-valued entry is invalid as a scalar operand, including a structured SubQuery whose known, non-wildcard projection contains more than one column
- **Examples:**
  - ```java
    User user = new User("John", "john@example.com", 25);
...
- **Returns:** an Or condition
- **Throws:**
  - `java.lang.IllegalArgumentException` — if entities or includedPropNames is null or empty, all entities are null, an element is a map, or a property name is null, empty, blank, or not readable; if a condition-valued entry is invalid as a scalar operand, including a structured SubQuery whose known, non-wildcard projection contains more than one column
- **Examples:**
  - ```java
...
- **Parameters:**
  - `propName` (`String`) — the property/column name
  - `minValue` (`Object`) — the minimum value (exclusive, must not be null)
...
- **Parameters:**
  - `propName` (`String`) — the property/column name
  - `minValue` (`Object`) — the minimum value (inclusive, must not be null)
  - `maxValue` (`Object`) — the maximum value (exclusive, must not be null)
- **Returns:** an And condition
- **Throws:**
  - `java.lang.IllegalArgumentException` — if a scalar subquery operand has a known, non-wildcard projection with more than one column; if propName is null, empty, or blank, if minValue or maxValue is null or a blank SqlExpression, or if either is any other Condition (an ordinary predicate, Criteria, clause, JOIN or ON/USING connector, or a nested All/Any/Some operand)
...
- **Summary:** Converts an EntityId to an And condition where each key-value pair becomes an equality check.
- **Deprecated:** the digit-abbreviation name is inconsistent with this class's spelled-out naming convention; use #idToCond(EntityId) instead
- **Parameters:**
...
- **Summary:** Converts a collection of EntityIds to an Or condition where each EntityId becomes an And condition.
- **Deprecated:** the digit-abbreviation name is inconsistent with this class's spelled-out naming convention; use #idToCond(Collection) instead
- **Parameters:**
  - `entityIds` (`Collection<? extends EntityId>`) — collection of EntityIds (must not be null, empty, or contain null)
- **Returns:** an Or condition
- **Throws:**
...
- **Parameters:**
  - `propName` (`String`) — the property/column name
  - `minValue` (`Object`) — the minimum value of the excluded range (inclusive, must not be null)
...
- **Parameters:**
  - `propName` (`String`) — the property/column name
  - `pattern` (`String`) — the pattern to match (can include SQL wildcards); must not be null
- **Returns:** a Like condition
- **Throws:**
  - `java.lang.IllegalArgumentException` — if propName is null, empty, or blank, or if pattern is null
- **Examples:**
  - ```java
...
- **Parameters:**
  - `propName` (`String`) — the property/column name
  - `pattern` (`String`) — the pattern to exclude (can include SQL wildcards); must not be null
...
- **Contract:**
  ...
  - Otherwise propValue must be a Boolean or an SqlExpression naming the desired SQL keyword.
- **Parameters:**
  - `propName` (`String`) — the property/column name (must not be null, empty, or blank)
...
- **Examples:**
  - ```java
    ...
    ```
##### isTrue(...) -> Is
- **Signature:** `public static Is isTrue(final String propName)`
- **Summary:** Creates an IS TRUE predicate without turning the boolean literal into a bind parameter.
- **Contract:**
...
- **Throws:**
  - `java.lang.IllegalArgumentException` — if propName is null, empty, or blank
##### isFalse(...) -> Is
- **Signature:** `public static Is isFalse(final String propName)`
- **Summary:** Creates an IS FALSE predicate without turning the boolean literal into a bind parameter.
- **Contract:**
...
- **Throws:**
  - `java.lang.IllegalArgumentException` — if any element of conditions is null, or is a Criteria, a clause (WHERE, JOIN variants, ORDER BY, etc.), an ON/USING connector, an ANY/ALL/SOME quantified-subquery operand, a standalone SubQuery, or a blank SqlExpression
- **Examples:**
  - ```java
...
- **Throws:**
  - `java.lang.NullPointerException` — if operator is null
  - `java.lang.IllegalArgumentException` — if operator is not Operator#AND or Operator#OR, or if any element of conditions is null, or is a Criteria, a clause (WHERE, JOIN variants, ORDER BY, etc.), an ON/USING connector, an ANY/ALL/SOME quantified-subquery operand, a standalone SubQuery, or a blank SqlExpression
- **Examples:**
  - ```java
...
- **Contract:**
  - When rendered by a configured SqlBuilder, detected identifiers may still be converted according to its naming policy.
- **Parameters:**
...
- **Contract:**
  - Use an OrderBy clause when the result rows require a defined order.
- **Parameters:**
...
- **Returns:** a GroupBy clause
- **Throws:**
  - `java.lang.IllegalArgumentException` — if any property name is null, empty, or blank, if the property names are not distinct, or if any sort direction is null
- **Examples:**
  - ```java
...
- **Summary:** Creates an On clause for a simple equality join between two columns.
- **Parameters:**
...
- **Examples:**
  - ```java
    ...
    ```
##### using(...) -> Using
- **Signature:** `@Beta public static Using using(final String... columnNames)`
...
- **Examples:**
  - ```java
    ...
    ```
- **Signature:** `@Beta public static Using using(final Collection<String> columnNames)`
...
- **Examples:**
  - ```java
    ...
    ```
##### join(...) -> Join
- **Signature:** `@Deprecated public static Join join(final String joinEntity)`
- **Summary:** Creates a Join clause for the specified entity/table without a join condition.
- **Deprecated:** always throws; use #join(String, Condition) or #crossJoin(String) instead
- **Parameters:**
  - `joinEntity` (`String`) — the entity/table name to join
- **Returns:** never returns normally
- **Throws:**
  - `java.lang.IllegalArgumentException` — always, because a qualified JOIN requires a non-null ON/USING predicate
- **Examples:**
  - ```java
    // Filters.join("orders") always throws; supply the predicate instead:
...
- **Examples:**
  - ```java
    ...
    // Results in SQL like: JOIN (orders CROSS JOIN products) ON orders.product_id = products.id
    ```
##### leftJoin(...) -> LeftJoin
- **Signature:** `@Deprecated public static LeftJoin leftJoin(final String joinEntity)`
- **Summary:** Creates a LeftJoin clause for the specified entity/table without a join condition.
- **Deprecated:** always throws; use #leftJoin(String, Condition) or #crossJoin(String) instead
- **Parameters:**
  - `joinEntity` (`String`) — the entity/table name to join
- **Returns:** never returns normally
- **Throws:**
  - `java.lang.IllegalArgumentException` — always, because a qualified LEFT JOIN requires a non-null ON/USING predicate
- **Examples:**
  - ```java
    // Filters.leftJoin("orders") always throws; supply the predicate instead:
    LeftJoin join = Filters.leftJoin("orders", Filters.on("users.id", "orders.user_id"));
...
- **Examples:**
  - ```java
    ...
    // Results in SQL like: LEFT JOIN (orders CROSS JOIN order_items) ON orders.id = order_items.order_id
    ```
##### rightJoin(...) -> RightJoin
- **Signature:** `@Deprecated public static RightJoin rightJoin(final String joinEntity)`
- **Summary:** Creates a RightJoin clause for the specified entity/table without a join condition.
- **Deprecated:** always throws; use #rightJoin(String, Condition) or #crossJoin(String) instead
- **Parameters:**
  - `joinEntity` (`String`) — the entity/table name to join
- **Returns:** never returns normally
- **Throws:**
  - `java.lang.IllegalArgumentException` — always, because a qualified RIGHT JOIN requires a non-null ON/USING predicate
- **Examples:**
  - ```java
    // Filters.rightJoin("users") always throws; supply the predicate instead:
    RightJoin join = Filters.rightJoin("users", Filters.on("orders.user_id", "users.id"));
...
- **Examples:**
  - ```java
    ...
    // Results in SQL like: RIGHT JOIN (departments CROSS JOIN locations) ON departments.location_id = locations.id
    ```
##### crossJoin(...) -> CrossJoin
...
- **Examples:**
  - ```java
    ...
    // Results in SQL like: CROSS JOIN (colors CROSS JOIN sizes)
    ```
##### fullJoin(...) -> FullJoin
- **Signature:** `@Deprecated public static FullJoin fullJoin(final String joinEntity)`
- **Summary:** Creates a FullJoin clause for the specified entity/table without a join condition.
- **Deprecated:** always throws; use #fullJoin(String, Condition) or #crossJoin(String) instead
- **Parameters:**
  - `joinEntity` (`String`) — the entity/table name to join
- **Returns:** never returns normally
- **Throws:**
  - `java.lang.IllegalArgumentException` — always, because a qualified FULL JOIN requires a non-null ON/USING predicate
- **Examples:**
  - ```java
    // Filters.fullJoin("employees") always throws; supply the predicate instead:
    FullJoin join = Filters.fullJoin("employees", Filters.on("departments.id", "employees.dept_id"));
...
- **Examples:**
  - ```java
    ...
    // Results in SQL like: FULL JOIN (employees CROSS JOIN contractors) ON employees.project_id = contractors.project_id
    ```
##### innerJoin(...) -> InnerJoin
- **Signature:** `@Deprecated public static InnerJoin innerJoin(final String joinEntity)`
- **Summary:** Creates an InnerJoin clause for the specified entity/table without a join condition.
- **Deprecated:** always throws; use #innerJoin(String, Condition) or #crossJoin(String) instead
- **Parameters:**
  - `joinEntity` (`String`) — the entity/table name to join
- **Returns:** never returns normally
- **Throws:**
  - `java.lang.IllegalArgumentException` — always, because a qualified INNER JOIN requires a non-null ON/USING predicate
- **Examples:**
  - ```java
    // Filters.innerJoin("products") always throws; supply the predicate instead:
    InnerJoin join = Filters.innerJoin("products", Filters.on("order_items.product_id", "products.id"));
...
- **Examples:**
  - ```java
    ...
    // Results in SQL like: INNER JOIN (orders CROSS JOIN order_details) ON orders.id = order_details.order_id
    ```
...
- **Examples:**
  - ```java
    ...
    // Results in SQL like: NATURAL JOIN (employees CROSS JOIN departments)
    ```
##### in(...) -> In
...
- **Contract:**
  ...
  - Every requested map key or bean property must exist, and every resolved tuple element must be non-null.
- **Parameters:**
...
- **Throws:**
  - `java.lang.IllegalArgumentException` — if propName is null, empty, or blank, if subQuery is null, or if subQuery is a structured subquery whose known, non-wildcard projection selects a number of columns other than 1
- **Examples:**
  - ```java
...
- **Returns:** an InSubQuery condition
- **Throws:**
  - `java.lang.IllegalArgumentException` — if propNames is null or empty or contains a null, empty, or blank name, if subQuery is null, or if subQuery is a structured subquery whose known, non-wildcard projection selects a number of columns that does not match propNames.size()
- **Examples:**
  - ```java
...
- **Deprecated:** when the full SQL is supplied, entityName is not used to build the subquery; use #subQuery(String) instead.
- **Parameters:**
...
- **Returns:** a SubQuery
- **Throws:**
  - `java.lang.IllegalArgumentException` — if sql is null, empty, or blank, or contains an unbound parameter placeholder
- **Examples:**
  - ```java
...
- **Examples:**
  - ```java
    ...
    ```
- **Signature:** `public static SubQuery subQuery(final String sql, final Collection<?> parameters)`
- **Summary:** Creates a raw SubQuery with positional JDBC bindings.
- **Contract:**
  - The complete SQL is retained verbatim; parameters is defensively copied and must have exactly one value for each positional ?
  - When this subquery is embedded in a SqlBuilder, those placeholders are converted to the builder's named or MyBatis parameter style, or their values are inlined for a raw-SQL policy.
  - As with every raw SQL API, the SQL structure itself must not come from untrusted input.
- **Parameters:**
...
- **Throws:**
  - `java.lang.IllegalArgumentException` — if the SQL is blank, parameters is null, a named/MyBatis marker is present, or the placeholder and binding counts differ
- **Examples:**
  - ```java
    SubQuery subQuery = Filters.subQuery(
...
- **Summary:** Creates a LIMIT clause from a string expression, normalizing and validating it against a fixed grammar.
- **Contract:**
...
- **Returns:** a Limit clause
- **Throws:**
  - `java.lang.IllegalArgumentException` — if expr is null, empty, blank, contains a placeholder, or is not one of the accepted forms
- **Examples:**
  - ```java
...
- **Summary:** Returns the parameterized SQL with named parameters replaced by JDBC placeholders (?) for recognized data-operation statements.
- **Parameters:**
  - (none)
...
- **Summary:** Returns the list of named parameters extracted from the SQL in order of appearance.
- **Contract:**
  - The list is empty if the SQL has no named parameters, or if the SQL is not a recognized data operation statement (see the class-level documentation), in which case no parameter extraction is performed.
- **Parameters:**
  - (none)
...
- **Summary:** Returns the total number of parameters (named or positional) in the SQL.
- **Parameters:**
  - (none)
- **Returns:** the number of parameters in the SQL; this can be smaller than the number of ? characters in #parameterizedSql() when an operator or quoted text such as \[what?\] contains a question mark
- **Examples:**
  - ```java
...
#### Public Fields
- **Declaration:** `public static final Pattern SIMPLE_COLUMN_NAME_PATTERN`
  - **Summary:** Regular expression pattern for validating simple column names.
  - **Nullability:** unspecified
...
- **Contract:**
  - The ColumnInfo#isUnqualified() flag describes the mapped column value: it is true only when the column name is one bare identifier, so query builders may still prepend a table alias or sub-entity qualifier to it.
- **Parameters:**
...
- **Examples:**
  - ```java
    ...
    ```
##### convertIdentifier(...) -> String
- **Signature:** `public static String convertIdentifier(final String identifier, final NamingPolicy namingPolicy)`
- **Summary:** Converts a SQL identifier (a column or property name, optionally qualified such as t.firstName) with the given naming policy while preserving the leading and trailing underscore runs of every dot-separated segment.
- **Parameters:**
  - `identifier` (`String`) — the identifier to convert; null and empty strings are returned as-is
  - `namingPolicy` (`NamingPolicy`) — the naming policy to apply to the parts between the underscore runs; null and NamingPolicy#NO_CHANGE return identifier as-is
- **Returns:** the converted identifier with the leading and trailing underscore runs of every dot-separated segment preserved
- **Examples:**
  - ```java
...
- **Examples:**
  - ```java
    ...
    ```
- **See also:** #tableNameAndAlias(Class, NamingPolicy)
...
- **Parameters:**
  - (none)
- **Returns:** the immutable excluded property names; null or an empty set means no properties are excluded
##### hashCode(...) -> int
- **Signature:** `@Override public int hashCode()`
- **Summary:** Returns the hash code of this selection.
- **Parameters:**
  - (none)
- **Returns:** a hash code consistent with #equals(Object)
##### equals(...) -> boolean
- **Signature:** `@Override public boolean equals(final Object obj)`
- **Summary:** Compares this selection with another object for equality.
- **Contract:**
  - Two selections are equal only when every attribute is equal: the entity class, table alias, class alias, included property names, the sub-entity-properties flag, and excluded property names.
- **Parameters:**
  - `obj` (`Object`) — the object to compare with
- **Returns:** true if obj is a Selection with equal attributes, false otherwise
##### toString(...) -> String
- **Signature:** `@Override public String toString()`
- **Summary:** Returns a string representation of this selection listing all its attributes.
- **Parameters:**
  - (none)
- **Returns:** a string of the form "Selection(entityClass=..., tableAlias=..., classAlias=..., ...)"
...
- **Contract:**
  - null or an empty string means no alias; a non-empty alias is validated when the selection is rendered and must not be blank, quoted, or contain SQL comment tokens (it is prefixed verbatim to every rendered column).
- **Parameters:**
...
- **Summary:** Builds an immutable selection, defensively copying its property collections, including immutable wrappers over mutable backing collections.
- **Parameters:**
  - (none)
...
#### Public Fields
- **Declaration:** `public static final SortDirection ASC`
  - **Summary:** Ascending sort order (A to Z, 0 to 9, oldest to newest).
  - **Nullability:** unspecified
- **Declaration:** `public static final SortDirection DESC`
  - **Summary:** Descending sort order (Z to A, 9 to 0, newest to oldest).
  - **Nullability:** unspecified
...
#### Public Fields
- **Declaration:** `public static final BiConsumer<StringBuilder, String> DEFAULT_NAMED_PARAMETER_HANDLER`
  - **Summary:** Default renderer for SqlPolicy#NAMED_SQL placeholders.
  - **Nullability:** unspecified
...
#### Public Fields
- **Declaration:** `public static final IdentifierQuote DOUBLE_QUOTE`
  - **Summary:** ANSI/standard SQL double quote (").
  - **Nullability:** unspecified
- **Declaration:** `public static final IdentifierQuote BACKTICK`
  - **Summary:** MySQL/MariaDB-style backtick (`).
  - **Nullability:** unspecified
...
#### Public Fields
- **Declaration:** `public static final SqlPolicy RAW_SQL`
  - **Summary:** Inline values directly into the SQL string as literals.
  - **Nullability:** unspecified
- **Declaration:** `public static final SqlPolicy PARAMETERIZED_SQL`
  - **Summary:** Render each value as a positional ? placeholder and collect parameter values in order.
  - **Nullability:** unspecified
- **Declaration:** `public static final SqlPolicy NAMED_SQL`
  - **Summary:** Render values as named placeholders, such as :id or :firstName.
  - **Nullability:** unspecified
- **Declaration:** `public static final SqlPolicy IBATIS_SQL`
  - **Summary:** Render values as iBATIS/MyBatis-style named placeholders, such as #{id}.
  - **Nullability:** unspecified
...
- **Contract:**
  - Returns false (not comparable) when either this #version() or minVersion is null, blank, does not begin with an integer component, or has an empty or unparseable component (for example "8." or "8..0").
- **Parameters:**
...
- **Contract:**
  - Returns false (not comparable) when either this #version() or maxVersion is null, blank, does not begin with an integer component, or has an empty or unparseable component (for example "8." or "8..0").
- **Parameters:**
...
#### Public Fields
- **Declaration:** `public static final String SQL_MAPPER`
  - **Summary:** XML element name for the root sqlMapper element.
  - **Nullability:** unspecified
- **Declaration:** `public static final String SQL`
  - **Summary:** XML element name for individual sql elements.
  - **Nullability:** unspecified
- **Declaration:** `public static final String ID`
  - **Summary:** XML attribute name for the SQL identifier.
  - **Nullability:** unspecified
- **Declaration:** `public static final String BATCH_SIZE`
  - **Summary:** XML attribute name for batch size configuration.
  - **Nullability:** unspecified
- **Declaration:** `public static final String FETCH_SIZE`
  - **Summary:** XML attribute name for JDBC fetch size configuration.
  - **Nullability:** unspecified
- **Declaration:** `public static final String RESULT_SET_TYPE`
  - **Summary:** XML attribute name for JDBC result set type configuration.
  - **Nullability:** unspecified
- **Declaration:** `public static final ImmutableMap<String, Integer> RESULT_SET_TYPE_MAP`
  - **Summary:** Mapping of result set type names to their JDBC constant values.
  - **Nullability:** unspecified
- **Declaration:** `public static final String TIMEOUT`
  - **Summary:** XML attribute name for query timeout configuration.
  - **Nullability:** unspecified
- **Declaration:** `public static final int MAX_ID_LENGTH`
  - **Summary:** Maximum allowed length for SQL identifiers ({@value}).
  - **Nullability:** unspecified
...
- **Contract:**
  ...
  - The stream is consumed and must not be reused after this call returns.
- **Parameters:**
...
- **Throws:**
  - `java.lang.IllegalArgumentException` — if ParsedSql#parse(String) rejects sql (including null/blank SQL, mixed parameter styles, or malformed parameters), or if the id is null/empty, contains whitespace, exceeds #MAX_ID_LENGTH characters, or already exists
- **Examples:**
  - ```java
    SqlMapper mapper = new SqlMapper();
...
- **Parameters:**
  ...
  - `attributes` (`Map<String, String>`) — additional XML attributes for the SQL (e.g., batchSize, fetchSize, resultSetType, timeout); may be null or empty, but keys must be valid non-namespace XML attribute names and values must be non-null
- **Throws:**
  - `java.lang.IllegalArgumentException` — if ParsedSql#parse(String) rejects sql (including null/blank SQL, mixed parameter styles, or malformed parameters); if the id is null/empty, contains whitespace, exceeds #MAX_ID_LENGTH characters, or already exists; or if attributes contains a null/empty/invalid or namespace-qualified XML attribute name, or a null value
- **Examples:**
  - ```java
    SqlMapper mapper = new SqlMapper();
    Map<String, String> attrs = new HashMap<>();
...
#### Public Fields
- **Declaration:** `public static final SqlOperation SELECT`
  - **Summary:** SELECT operation for data retrieval.
  - **Nullability:** unspecified
- **Declaration:** `public static final SqlOperation INSERT`
  - **Summary:** INSERT operation for adding new records.
  - **Nullability:** unspecified
- **Declaration:** `public static final SqlOperation UPDATE`
  - **Summary:** UPDATE operation for modifying existing records.
  - **Nullability:** unspecified
- **Declaration:** `public static final SqlOperation DELETE`
  - **Summary:** DELETE operation for removing records.
  - **Nullability:** unspecified
- **Declaration:** `public static final SqlOperation MERGE`
  - **Summary:** MERGE operation for conditionally inserting, updating, or deleting records, depending on the database dialect.
  - **Nullability:** unspecified
- **Declaration:** `public static final SqlOperation CREATE`
  - **Summary:** CREATE operation for creating database objects.
  - **Nullability:** unspecified
- **Declaration:** `public static final SqlOperation DROP`
  - **Summary:** DROP operation for removing database objects.
  - **Nullability:** unspecified
- **Declaration:** `public static final SqlOperation ALTER`
  - **Summary:** ALTER operation for modifying database object structure.
  - **Nullability:** unspecified
- **Declaration:** `public static final SqlOperation SHOW`
  - **Summary:** SHOW operation for displaying database information.
  - **Nullability:** unspecified
- **Declaration:** `public static final SqlOperation DESCRIBE`
  - **Summary:** DESCRIBE operation for showing table structure.
  - **Nullability:** unspecified
- **Declaration:** `public static final SqlOperation USE`
  - **Summary:** USE operation for selecting a database.
  - **Nullability:** unspecified
- **Declaration:** `public static final SqlOperation RENAME`
  - **Summary:** RENAME operation for renaming database objects.
  - **Nullability:** unspecified
- **Declaration:** `public static final SqlOperation BEGIN_TRANSACTION`
  - **Summary:** BEGIN TRANSACTION operation for starting a transaction.
  - **Nullability:** unspecified
- **Declaration:** `public static final SqlOperation COMMIT`
  - **Summary:** COMMIT operation for committing a transaction.
  - **Nullability:** unspecified
- **Declaration:** `public static final SqlOperation ROLLBACK`
  - **Summary:** ROLLBACK operation for rolling back a transaction.
  - **Nullability:** unspecified
- **Declaration:** `public static final SqlOperation CALL`
  - **Summary:** CALL operation for executing stored procedures.
  - **Nullability:** unspecified
- **Declaration:** `public static final SqlOperation UNKNOWN`
  - **Summary:** UNKNOWN operation for unrecognized SQL commands.
  - **Nullability:** unspecified
...
- **Deprecated:** use TokenizerConfig#builder(); builder creation belongs to the type being built
- **Parameters:**
  - (none)
...
- **Signature:** `public static List<String> tokenize(final String sql)`
- **Summary:** Tokenizes a SQL statement into a list of lexical SQL tokens.
- **Parameters:**
  - `sql` (`String`) — the SQL statement to tokenize (must not be null)
- **Returns:** a list of tokens representing the tokenized SQL statement
- **Throws:**
...
- **Parameters:**
  - `tokens` (`List<String>`) — the parsed SQL tokens (typically the result of #tokenize(String))
...
- **Examples:**
  - ```java
    ...
    // result5 = false
    ```
- **See also:** #isInsertQuery(String), #isUpdateQuery(String), #isDeleteQuery(String), #isInsertOrReplaceQuery(String), #isSyntacticallyReadQuery(String), #isReadOrInsertQuery(String)
##### isSyntacticallyReadQuery(...) -> boolean
...
- **Parameters:**
  - `sql` (`String`) — the SQL statement to check; may be empty or null
- **Returns:** true if the SQL text has an accepted SELECT-only shape, false otherwise
...
- **Signature:** `@Deprecated public static boolean isReadOnlyQuery(final String sql)`
- **Summary:** Legacy alias for #isSyntacticallyReadQuery(String).
- **Deprecated:** The name overstates what lexical SQL inspection can prove. Use #isSyntacticallyReadQuery(String) and enforce actual read-only behavior with database privileges and transaction settings.
- **Parameters:**
  - `sql` (`String`) — the SQL statement to classify; may be empty or null
- **Returns:** the lexical SELECT-only classification
...
- **Examples:**
  - ```java
    ...
    // result5 = false
    ```
- **See also:** #isSelectQuery(String), #isUpdateQuery(String), #isDeleteQuery(String), #isInsertOrReplaceQuery(String), #isSyntacticallyReadQuery(String), #isReadOrInsertQuery(String)
...
- **Examples:**
  - ```java
    ...
    // result4 = false
    ```
- **See also:** #isSelectQuery(String), #isInsertQuery(String), #isDeleteQuery(String), #isInsertOrReplaceQuery(String), #isSyntacticallyReadQuery(String), #isReadOrInsertQuery(String)
...
- **Examples:**
  - ```java
    ...
    // result4 = false
    ```
- **See also:** #isSelectQuery(String), #isInsertQuery(String), #isUpdateQuery(String), #isInsertOrReplaceQuery(String), #isSyntacticallyReadQuery(String), #isReadOrInsertQuery(String)
...
- **Examples:**
  - ```java
    ...
    // result2 = false
    ```
- **See also:** #isSelectQuery(String), #isInsertQuery(String), #isUpdateQuery(String), #isDeleteQuery(String), #isSyntacticallyReadQuery(String), #isReadOrInsertQuery(String)
...
- **Parameters:**
  - `sql` (`String`) — the SQL statement to check; may be empty or null
- **Returns:** true for an accepted SELECT/plain-INSERT lexical shape; false otherwise, including for a null or empty statement
- **See also:** #isSelectQuery(String), #isInsertQuery(String), #isUpdateQuery(String), #isDeleteQuery(String), #isInsertOrReplaceQuery(String), #isSyntacticallyReadQuery(String)
...
##### semicolonlessBatches(...) -> boolean
- **Signature:** `public boolean semicolonlessBatches()`
- **Summary:** Returns whether the read gates (Tokenizer#isSyntacticallyReadQuery(String) and Tokenizer#isReadOrInsertQuery(String)) split SQL Server-style batches, in which a second statement follows the first without a ;.
- **Parameters:**
  - (none)
- **Returns:** true if semicolon-less batches are split at their statement verbs
- **See also:** Builder#withSemicolonlessBatches(boolean)
##### hashCode(...) -> int
- **Signature:** `@Override public int hashCode()`
- **Summary:** Returns the hash code of this configuration.
- **Parameters:**
  - (none)
- **Returns:** a hash code consistent with #equals(Object)
##### equals(...) -> boolean
- **Signature:** `@Override public boolean equals(final Object obj)`
- **Summary:** Compares this configuration with another object for equality.
- **Contract:**
  - Two configurations are equal when they hold equal separator sets and the same semicolon-less batch flag; the order in which separators were added does not affect equality.
- **Parameters:**
  - `obj` (`Object`) — the object to compare with
- **Returns:** true if obj is a TokenizerConfig with the same separators and flag, false otherwise
##### toString(...) -> String
- **Signature:** `@Override public String toString()`
- **Summary:** Returns a string representation of this configuration listing its separators and the semicolon-less batch flag.
- **Parameters:**
  - (none)
- **Returns:** a string of the form "TokenizerConfig{separators=\[...\], semicolonlessBatches=false}"
...
#### Public Instance Methods
##### withSemicolonlessBatches(...) -> Builder
...
##### tokenize(...) -> List<String>
- **Signature:** `public List<String> tokenize(final String sql)`
- **Summary:** Tokenizes a SQL statement with this tokenizer's separator configuration.
- **Parameters:**
  - `sql` (`String`) — the SQL statement to tokenize; must not be null
...
- **Examples:**
  - ```java
    ...
    ```
- **See also:** SqlParser#tokenize(String)
##### indexOfToken(...) -> int
...
##### isSyntacticallyReadQuery(...) -> boolean
- **Signature:** `public boolean isSyntacticallyReadQuery(final String sql)`
- **Summary:** Lexically classifies the statement as SELECT-only using this tokenizer's configured separator rules and its semicolon-less batch setting.
- **Contract:**
  - This method does not resolve functions, triggers, locking clauses, or other database semantics and must not be used as a security boundary.
- **Parameters:**
  - `sql` (`String`) — the SQL statement to classify; may be empty or null
- **Returns:** true if the SQL text has an accepted SELECT-only shape, false otherwise
- **See also:** SqlParser#isSyntacticallyReadQuery(String)
##### isReadOnlyQuery(...) -> boolean
- **Signature:** `@Deprecated public boolean isReadOnlyQuery(final String sql)`
- **Summary:** Legacy alias for #isSyntacticallyReadQuery(String).
- **Deprecated:** The name overstates what lexical SQL inspection can prove. Use #isSyntacticallyReadQuery(String) and enforce actual read-only behavior with database privileges and transaction settings.
- **Parameters:**
  - `sql` (`String`) — the SQL statement to classify; may be empty or null
- **Returns:** the lexical SELECT-only classification
##### isReadOrInsertQuery(...) -> boolean
- **Signature:** `public boolean isReadOrInsertQuery(final String sql)`
- **Summary:** Lexically classifies the statement as SELECT-or-INSERT-only using this tokenizer's configured separator rules and its semicolon-less batch setting; otherwise identical to SqlParser#isReadOrInsertQuery(String).
- **Contract:**
  - This method does not resolve functions, triggers or other database semantics and must not be used as a security boundary.
- **Parameters:**
  - `sql` (`String`) — the SQL statement to classify; may be empty or null
- **Returns:** true if the SQL text has an accepted SELECT-or-INSERT-only shape, false otherwise
- **See also:** SqlParser#isReadOrInsertQuery(String), #isSyntacticallyReadQuery(String)
...
- **Parameters:**
  - (none)
- **Returns:** the configured minimum value, which may be a literal, a SqlExpression, or a scalar SubQuery; known mutable JDK values are returned as defensive copies; null is returned only for an uninitialized serialization-framework instance
- **Examples:**
  - ```java
    Between between = new Between("age", 18, 65);
...
- **Parameters:**
  - (none)
- **Returns:** the configured maximum value, which may be a literal, a SqlExpression, or a scalar SubQuery; known mutable JDK values are returned as defensive copies; null is returned only for an uninitialized serialization-framework instance
- **Examples:**
  - ```java
    Between between = new Between("age", 18, 65);
...
- **Contract:**
  ...
  - Usage Examples: The result is memoized only when both bounds are plain scalars (neither an array, Date, Calendar nor a nested Condition); otherwise a fresh list, holding fresh defensive copies of any array/Date/Calendar values (including those spliced in from a nested condition), is built on every call.
- **Parameters:**
  - (none)
- **Returns:** an immutable list containing \[minValue, maxValue\], or their respective parameters spliced in where a bound is itself a Condition; known mutable JDK bounds are returned as defensive copies
- **Examples:**
  - ```java
...
- **Parameters:**
  - (none)
- **Returns:** an immutable list of the values (or value tuples), or an empty immutable list for an uninitialized instance; array, Date and Calendar elements in the list are defensive copies (a fresh list is built on every call when any element is an array, Date, Calendar or nested Condition; the memoized view is reused only when every element is a plain scalar), so mutating one of these defensive copies never affects this condition. Other mutable values remain shared by reference
- **Examples:**
  - ```java
    In inCond = new In("status", Arrays.asList("active", "pending"));
...
- **Contract:**
  - Usage Examples: The result is memoized only when every membership value is a plain scalar (neither an array, Date, Calendar nor a nested Condition); otherwise a fresh list, holding fresh defensive copies of any array/Date/Calendar values (including those spliced in from a nested condition), is built on every call.
- **Parameters:**
  - (none)
- **Returns:** an immutable list of parameter values, or an empty immutable list for an uninitialized instance (e.g. created via the no-arg constructor for deserialization); array, Date and Calendar values in the list are defensive copies (the list is rebuilt on every call in that case rather than memoized), so mutating one of these defensive copies never affects this condition or a later call. Other mutable values remain shared by reference
- **Examples:**
  - ```java
...
- **Parameters:**
  - (none)
- **Returns:** an immutable list of property names (never null; empty for an uninitialized instance)
- **Examples:**
  - ```java
    SubQuery subQuery = new SubQuery("SELECT id FROM departments WHERE active = true");
    InSubQuery inSub = new InSubQuery("dept_id", subQuery);
...
- **Parameters:**
  - `subQuery` (`SubQuery`) — the subquery that returns values to compare against (must not be null); a structured subquery with an explicit, non-wildcard projection must select exactly one property
- **Throws:**
  - `java.lang.IllegalArgumentException` — if subQuery is null, or if it is a structured subquery with a known, non-wildcard projection arity other than one
...
- **Throws:**
  - `java.lang.IllegalArgumentException` — if condition is null, or if condition is or contains a non-composable component — a Criteria, a Clause condition (such as Where or OrderBy), an ON/USING connector, an ANY/ALL/SOME quantified-subquery operand, a standalone SubQuery, or a blank SqlExpression
- **Examples:**
  - ```java
...
- **Throws:**
  - `java.lang.IllegalArgumentException` — if propName is null, empty, or blank; if operator is not one of the operators listed above; or if, for an IN/NOT_IN operator, propValue is not a non-empty Collection, a non-empty array, a SqlExpression, or a SubQuery; if a value that must be non-null is null; if IS/IS NOT receives an arbitrary literal; if a condition-valued operand is an ordinary predicate or query clause or a blank SqlExpression; or if an All/Any/Some operand is used anywhere other than the direct RHS of a compatible scalar comparison; or if a scalar SubQuery has a known, non-wildcard projection with multiple columns
  - `java.lang.NullPointerException` — if operator is null
...
- **Contract:**
  - A collection used as a scalar value by another operator retains its concrete type and identity; callers must not mutate that collection while the condition is in use.
- **Parameters:**
  - (none)
...
- **Returns:** the property value cast to valueType, or null when the stored value is null; known mutable JDK values are returned as defensive copies
- **Throws:**
...
- **Contract:**
  ...
  - Usage Examples: The result is memoized only when every operand is a plain scalar (neither an array, Date, Calendar nor a nested Condition); otherwise a fresh list, holding fresh defensive copies of any array/Date/Calendar values (including those spliced in from a nested condition), is built on every call.
  - Other mutable scalar values, including collections used outside IN/NOT IN, are retained by reference and must not be mutated while in use.
- **Parameters:**
  - (none)
- **Returns:** an immutable list of parameter values; known mutable JDK values in the list are defensive copies, and the result is never null
- **Examples:**
  - ```java
...
A composable variant of Cell that supports logical composition via AND/OR/NOT/XOR operations.
...
- **Throws:**
  - `java.lang.IllegalArgumentException` — if condition is null, or if either this or condition is or contains a non-composable component — a Criteria, a SQL clause, an ON/USING connector, an ANY/ALL/SOME quantified-subquery operand, a standalone SubQuery, or a blank SqlExpression
- **Examples:**
  - ```java
...
- **Parameters:**
  - (none)
- **Returns:** the SELECT modifier with any surrounding whitespace stripped (an empty or blank modifier is normalized to null, so this does not necessarily round-trip the string that was set), or null if not set
- **Examples:**
  - ```java
    ...
    ```
- **See also:** Builder#distinct(), Builder#distinctOn(String), Builder#distinctRow(), Builder#selectModifier(String)
...
- **Parameters:**
  - `namingPolicy` (`NamingPolicy`) — the naming policy to apply to property names within each clause; null is normalized to NamingPolicy#NO_CHANGE before rendering each clause
...
- **Contract:**
  - Two instances are equal if they have the exact same runtime class, select modifier, and ordered list of conditions.
- **Parameters:**
  - `obj` (`Object`) — the object to compare with
- **Returns:** true if the objects are equal, false otherwise
- **Examples:**
...
- **Parameters:**
  - `selectModifier` (`String`) — the custom SELECT modifier; null, empty, or blank means no modifier, and any surrounding whitespace is stripped
- **Returns:** this Builder instance for method chaining
- **Examples:**
  - ```java
...
- **Examples:**
  - ```java
    ...
    empty.joins();      // returns [] (empty list)
    ```
- **Signature:** `@Deprecated public Builder join(final String joinEntity)`
- **Summary:** Adds a plain JOIN (no explicit type keyword) to this criteria, without a join condition.
- **Deprecated:** always throws IllegalArgumentException; use #join(String, Condition) or #crossJoin(String) instead.
- **Parameters:**
...
- **Throws:**
  - `java.lang.IllegalArgumentException` — always, because a qualified join requires an ON/USING predicate
- **Examples:**
  - ```java
...
- **Returns:** this Builder instance for method chaining
- **Throws:**
  - `java.lang.IllegalArgumentException` — if joinEntity is null, empty, or blank, if joinCondition is null (qualified joins require an ON/USING predicate), or if joinCondition is not valid for a JOIN
- **Examples:**
  - ```java
    Criteria criteria = Criteria.builder()
...
- **Returns:** this Builder instance for method chaining
- **Throws:**
  - `java.lang.IllegalArgumentException` — if joinEntities is null or empty, contains null, empty, or blank elements, if joinCondition is null (qualified joins require an ON/USING predicate), or if joinCondition is not valid for a JOIN
- **Examples:**
  - ```java
    ...
    ```
##### innerJoin(...) -> Builder
- **Signature:** `@Deprecated public Builder innerJoin(final String joinEntity)`
- **Summary:** Adds an INNER JOIN to this criteria, without a join condition.
- **Deprecated:** always throws IllegalArgumentException; use #innerJoin(String, Condition) or #crossJoin(String) instead.
- **Parameters:**
  - `joinEntity` (`String`) — the table or entity to join
- **Returns:** this Builder instance for method chaining (never reached; this overload always throws)
- **Throws:**
...
- **Examples:**
  - ```java
    ...
    ```
##### leftJoin(...) -> Builder
- **Signature:** `@Deprecated public Builder leftJoin(final String joinEntity)`
- **Summary:** Adds a LEFT JOIN to this criteria, without a join condition.
- **Deprecated:** always throws IllegalArgumentException; use #leftJoin(String, Condition) or #crossJoin(String) instead.
- **Parameters:**
  - `joinEntity` (`String`) — the table or entity to join
- **Returns:** this Builder instance for method chaining (never reached; this overload always throws)
- **Throws:**
...
- **Examples:**
  - ```java
    ...
    ```
##### rightJoin(...) -> Builder
- **Signature:** `@Deprecated public Builder rightJoin(final String joinEntity)`
- **Summary:** Adds a RIGHT JOIN to this criteria, without a join condition.
- **Deprecated:** always throws IllegalArgumentException; use #rightJoin(String, Condition) or #crossJoin(String) instead.
- **Parameters:**
  - `joinEntity` (`String`) — the table or entity to join
- **Returns:** this Builder instance for method chaining (never reached; this overload always throws)
- **Throws:**
...
- **Examples:**
  - ```java
    ...
    ```
##### fullJoin(...) -> Builder
- **Signature:** `@Deprecated public Builder fullJoin(final String joinEntity)`
- **Summary:** Adds a FULL JOIN to this criteria, without a join condition.
- **Deprecated:** always throws IllegalArgumentException; use #fullJoin(String, Condition) or #crossJoin(String) instead.
- **Parameters:**
  - `joinEntity` (`String`) — the table or entity to join
- **Returns:** this Builder instance for method chaining (never reached; this overload always throws)
- **Throws:**
...
- **Returns:** this Builder instance for method chaining
- **Throws:**
  - `java.lang.IllegalArgumentException` — if any property name is null, empty, or blank, if the two property names are equal (duplicate property name), or if any sort direction is null
- **Examples:**
  - ```java
    Criteria.Builder builder = Criteria.builder()
...
- **Returns:** this Builder instance for method chaining
- **Throws:**
  - `java.lang.IllegalArgumentException` — if any property name is null, empty, or blank, if any two property names are equal (duplicate property name), or if any sort direction is null
- **Examples:**
  - ```java
    Criteria.Builder builder = Criteria.builder()
...
- **Returns:** this Builder instance for method chaining
- **Throws:**
  - `java.lang.IllegalArgumentException` — if condition is null, is a Criteria, uses ON/USING, is an empty predicate (a blank SqlExpression), is an ANY/ALL/SOME quantified operand, is a standalone SubQuery, or is a clause condition with an operator other than HAVING (an SqlExpression whose literal begins with a clause keyword counts as a clause condition and is rejected as well; pass only the predicate text)
- **Examples:**
  - ```java
    Criteria c = Criteria.builder()
        .groupBy("department")
...
- **Returns:** this Builder instance for method chaining
- **Throws:**
  - `java.lang.IllegalArgumentException` — if condition is null, is a Criteria, uses ON/USING, is an empty predicate (a blank SqlExpression), is an ANY/ALL/SOME quantified operand, is a standalone SubQuery, or is a clause condition with an operator other than ORDER_BY (an SqlExpression whose literal begins with a clause keyword counts as a clause condition and is rejected as well; pass only the ordering text). An empty Junction is accepted and renders its Boolean identity (for example ORDER BY 1 = 0)
- **Examples:**
  - ```java
...
- **Examples:**
  - ```java
    ...
    ```
##### limit(...) -> Builder
- **Signature:** `public Builder limit(final Limit limit)`
...
- **Returns:** this Builder instance for method chaining
- **Throws:**
  - `java.lang.IllegalArgumentException` — if expr is null, empty, blank, not an accepted limit form, or contains a placeholder
- **Examples:**
  - ```java
...
#### Public Constructors
- **Signature:** `@Deprecated public FullJoin(final String joinEntity)`
- **Summary:** Creates a FULL JOIN clause for the specified table or entity without a join condition.
- **Deprecated:** always throws IllegalArgumentException because a qualified join requires an ON/USING predicate; use #FullJoin(String, Condition) instead, or CrossJoin for an unconditional join
- **Parameters:**
  - `joinEntity` (`String`) — the table or entity to join with. Can include alias (e.g., "orders o").
- **Throws:**
...
- **Throws:**
  - `java.lang.IllegalArgumentException` — if propName is null, empty, or blank; propValue is null, an ordinary predicate or query clause, or a blank SqlExpression; or if a structured subquery has a known, non-wildcard projection with a column count other than one
...
Represents a greater-than-or-equal-to (&gt;=) comparison condition in SQL queries.
...
- **Throws:**
  - `java.lang.IllegalArgumentException` — if propOrColumnName is null, empty, or blank, or if direction is null
...
- **Throws:**
  - `java.lang.IllegalArgumentException` — if propName is null, empty, or blank, if values is null, empty, or contains null, or if any element is a Condition other than a non-blank SqlExpression or a scalar SubQuery (predicates, clauses, Criteria, JOIN/ON/USING connectors and All/Any/Some quantified operands are all rejected), or if a scalar SubQuery has a known, non-wildcard projection containing multiple columns
...
- **Parameters:**
  - `propName` (`String`) — the property/column name (must not be null, empty, or blank)
  - `subQuery` (`SubQuery`) — the subquery that returns the values to check against (must not be null); if it has an explicit structured projection without * or qualifier.*, it must select exactly one column; raw SQL and wildcard projections cannot be arity-checked here
- **Throws:**
  - `java.lang.IllegalArgumentException` — if propName is null, empty, or blank, if subQuery is null, or if the subquery has a known, non-wildcard structured projection whose column count is not 1
...
- **Parameters:**
  - `propNames` (`Collection<String>`) — the property/column names to check (must not be null or empty and must not contain null, empty, or blank elements). Their order must match the column order in the subquery.
  - `subQuery` (`SubQuery`) — the subquery that returns the values to check against (must not be null). If it has an explicit structured projection without * or qualifier.*, it must select exactly propNames.size() columns. Raw SQL and wildcard projections cannot be arity-checked here.
- **Throws:**
  - `java.lang.IllegalArgumentException` — if propNames is null or empty, if any element is null, empty, or blank, if subQuery is null, or if the subquery has a known, non-wildcard structured projection whose number of selected columns does not match propNames.size()
...
#### Public Constructors
- **Signature:** `@Deprecated public InnerJoin(final String joinEntity)`
- **Summary:** Creates an INNER JOIN clause for the specified table or entity without a join condition.
- **Deprecated:** always throws IllegalArgumentException because a qualified join requires an ON/USING predicate; use #InnerJoin(String, Condition) instead, or CrossJoin for an unconditional join
- **Parameters:**
  - `joinEntity` (`String`) — the table or entity to join with. Can include alias (e.g., "orders o").
- **Throws:**
...
- **Parameters:**
  - `subQuery` (`SubQuery`) — the subquery to perform the INTERSECT operation with (must not be null). The subquery must have the same number of columns with compatible types as the main query.
- **Throws:**
  - `java.lang.IllegalArgumentException` — if subQuery is null
...
#### Public Constructors
- **Signature:** `@Deprecated public Join(final String joinEntity)`
- **Summary:** Creates a plain JOIN clause for the specified table or entity without a join condition.
- **Deprecated:** always throws IllegalArgumentException because a qualified join requires an ON/USING predicate; use #Join(String, Condition) instead, or CrossJoin for an unconditional join
- **Parameters:**
  - `joinEntity` (`String`) — the table or entity to join with. Can include alias (e.g., "orders o").
- **Throws:**
...
- **Signature:** `public boolean hasCondition()`
- **Summary:** Returns whether this join has a condition.
- **Contract:**
  - Returns true if a condition was supplied at construction time, false otherwise.
- **Parameters:**
  - (none)
- **Returns:** true if this join has a condition, false otherwise
- **Examples:**
  - ```java
    // Join with ON condition
    Join join = new Join("orders o", new On("customers.id", "o.customer_id"));
...
- **Parameters:**
  - `namingPolicy` (`NamingPolicy`) — the naming policy passed through to the join condition's Condition#toSql(NamingPolicy) method; if null, it is normalized to NamingPolicy#NO_CHANGE before delegation
...
#### Public Constructors
- **Signature:** `@Deprecated public LeftJoin(final String joinEntity)`
- **Summary:** Creates a LEFT JOIN clause for the specified table or entity without a join condition.
- **Deprecated:** always throws IllegalArgumentException because a qualified join requires an ON/USING predicate; use #LeftJoin(String, Condition) instead, or CrossJoin for an unconditional join
- **Parameters:**
  - `joinEntity` (`String`) — the table or entity to join with. Can include alias (e.g., "orders o").
- **Throws:**
...
Represents a less-than-or-equal-to (&lt;=) comparison condition in SQL queries.
...
- **Throws:**
  - `java.lang.IllegalArgumentException` — if propName is null, empty, or blank, or propValue is null, an ordinary predicate or query clause, a blank SqlExpression, or a quantified All/Any/Some operand; or if a structured subquery has a known, non-wildcard projection with a column count other than one
...
- **Summary:** Creates LIMIT count with no offset.
- **Parameters:**
  - `count` (`int`) — maximum number of rows to return; must be non-negative
- **Throws:**
  - `java.lang.IllegalArgumentException` — if count is negative
- **Signature:** `public Limit(final int count, final int offset)`
- **Summary:** Creates LIMIT count OFFSET offset.
- **Parameters:**
  - `count` (`int`) — maximum number of rows to return; must be non-negative
  - `offset` (`int`) — number of rows to skip; must be non-negative
- **Throws:**
  - `java.lang.IllegalArgumentException` — if count or offset is negative
- **Signature:** `public Limit(final String expr)`
- **Summary:** Creates a row-limiting clause from a validated SQL expression.
- **Parameters:**
  - `expr` (`String`) — the row-limiting expression. Must not be null, empty, or blank, and must match a supported form.
- **Throws:**
  - `java.lang.IllegalArgumentException` — if expr is null, empty, or blank, or is not a supported limit form
...
- **Returns:** true if obj has the same runtime class, operator, and representation
...
- **Throws:**
  - `java.lang.IllegalArgumentException` — if propName is null, empty, or blank (whitespace-only)
...
- **Throws:**
  - `java.lang.IllegalArgumentException` — if value is a blank SqlExpression, a Condition other than an SqlExpression, a scalar SubQuery or a direct All/Any/Some operand, or a structured SubQuery whose known, non-wildcard projection does not contain exactly one column
- **Examples:**
  - ```java
...
- **Throws:**
  - `java.lang.IllegalArgumentException` — if values is null or empty, or if an element is a blank SqlExpression, a Condition other than an SqlExpression, a scalar SubQuery or a direct All/Any/Some operand, or a structured SubQuery whose known, non-wildcard projection does not contain exactly one column
- **Examples:**
  - ```java
...
- **Signature:** `public Or equalsAny(final boolean[] values)`
- **Summary:** Creates an OR condition with multiple EQUAL checks for this property using primitive boolean values.
- **Parameters:**
...
- **Examples:**
  - ```java
    ...
    ```
- **See also:** Or, Equal
- **Signature:** `public Or equalsAny(final char[] values)`
- **Summary:** Creates an OR condition with multiple EQUAL checks for this property using primitive char values.
- **Parameters:**
...
- **Examples:**
  - ```java
    ...
    ```
- **See also:** Or, Equal
- **Signature:** `public Or equalsAny(final byte[] values)`
- **Summary:** Creates an OR condition with multiple EQUAL checks for this property using primitive byte values.
- **Parameters:**
...
- **Examples:**
  - ```java
    ...
    ```
- **See also:** Or, Equal
- **Signature:** `public Or equalsAny(final short[] values)`
- **Summary:** Creates an OR condition with multiple EQUAL checks for this property using primitive short values.
- **Parameters:**
...
- **Examples:**
  - ```java
    ...
    ```
- **See also:** Or, Equal
- **Signature:** `public Or equalsAny(final float[] values)`
- **Summary:** Creates an OR condition with multiple EQUAL checks for this property using primitive float values.
- **Parameters:**
...
- **Returns:** an Or condition containing multiple Equal conditions
- **Throws:**
  - `java.lang.IllegalArgumentException` — if values is null or empty, or yields no elements when iterated, or if an element is a blank SqlExpression, a Condition other than an SqlExpression, a scalar SubQuery or a direct All/Any/Some operand, or a structured SubQuery whose known, non-wildcard projection does not contain exactly one column
- **Examples:**
  - ```java
...
- **Throws:**
  - `java.lang.IllegalArgumentException` — if value is null (use isNull()/isNotNull() instead), a blank SqlExpression, a Condition other than an SqlExpression, a scalar SubQuery or a direct All/Any/Some operand, or a structured SubQuery whose known, non-wildcard projection does not contain exactly one column
- **Examples:**
  - ```java
...
- **Throws:**
  - `java.lang.IllegalArgumentException` — if minValue or maxValue is null, a blank SqlExpression, a Condition other than an SqlExpression or a scalar SubQuery, or a structured SubQuery whose known, non-wildcard projection does not contain exactly one column
- **Examples:**
  - ```java
...
- **Parameters:**
  - `minValue` (`Object`) — the lower bound of the excluded range (this boundary is excluded when the bounds are ordered). Can be numeric, date, string, or any comparable type.
...
- **Parameters:**
  - `value` (`String`) — the pattern to match (can include % for any characters and _ for single character); must not be null
...
- **Parameters:**
  - `value` (`String`) — the pattern to exclude (can include % for any characters and _ for single character); must not be null
...
- **Throws:**
  - `java.lang.IllegalArgumentException` — if values is null or empty, contains a null element, or contains a Condition other than a non-blank SqlExpression or a scalar SubQuery
- **Examples:**
  - ```java
...
- **Signature:** `public In in(final boolean[] values)`
- **Summary:** Creates an IN condition for this property with primitive boolean values.
- **Parameters:**
...
- **Examples:**
  - ```java
    ...
    ```
- **See also:** In, Filters#in(String, boolean\[\])
- **Signature:** `public In in(final char[] values)`
- **Summary:** Creates an IN condition for this property with primitive char values.
- **Parameters:**
...
- **Examples:**
  - ```java
    ...
    // SQL: grade IN ('A', 'B', 'C')
    ```
- **See also:** In, Filters#in(String, char\[\])
- **Signature:** `public In in(final byte[] values)`
- **Summary:** Creates an IN condition for this property with primitive byte values.
- **Parameters:**
...
- **Examples:**
  - ```java
    ...
    // SQL: flag IN (0, 1, 2)
    ```
- **See also:** In, Filters#in(String, byte\[\])
- **Signature:** `public In in(final short[] values)`
- **Summary:** Creates an IN condition for this property with primitive short values.
- **Parameters:**
...
- **Examples:**
  - ```java
    ...
    // SQL: level IN (1, 2, 3)
    ```
- **See also:** In, Filters#in(String, short\[\])
...
- **Signature:** `public In in(final float[] values)`
- **Summary:** Creates an IN condition for this property with primitive float values.
- **Parameters:**
...
- **Examples:**
  - ```java
    ...
    ```
- **See also:** In, Filters#in(String, float\[\])
...
- **Signature:** `public NotIn notIn(final boolean[] values)`
- **Summary:** Creates a NOT IN condition for this property with primitive boolean values.
- **Parameters:**
...
- **Examples:**
  - ```java
    ...
    ```
- **See also:** NotIn, Filters#notIn(String, boolean\[\])
- **Signature:** `public NotIn notIn(final char[] values)`
- **Summary:** Creates a NOT IN condition for this property with primitive char values.
- **Parameters:**
...
- **Examples:**
  - ```java
    ...
    ```
- **See also:** NotIn, Filters#notIn(String, char\[\])
- **Signature:** `public NotIn notIn(final byte[] values)`
- **Summary:** Creates a NOT IN condition for this property with primitive byte values.
- **Parameters:**
...
- **Examples:**
  - ```java
    ...
    // SQL: flag NOT IN (0, 1)
    ```
- **See also:** NotIn, Filters#notIn(String, byte\[\])
- **Signature:** `public NotIn notIn(final short[] values)`
- **Summary:** Creates a NOT IN condition for this property with primitive short values.
- **Parameters:**
...
- **Examples:**
  - ```java
    ...
    ```
- **See also:** NotIn, Filters#notIn(String, short\[\])
...
- **Signature:** `public NotIn notIn(final float[] values)`
- **Summary:** Creates a NOT IN condition for this property with primitive float values.
- **Parameters:**
...
- **Examples:**
  - ```java
    ...
    ```
- **See also:** NotIn, Filters#notIn(String, float\[\])
...
- **Parameters:**
  - `obj` (`Object`) — the object to compare with
- **Returns:** true if the objects have the same runtime class and property name, false otherwise
- **Examples:**
  - ```java
...
- **Parameters:**
  - `condition` (`Condition`) — the condition to be negated; must not be null. May be any composable condition, including simple comparisons, logical junctions (And, Or), or subquery predicates such as Exists, NotExists, or InSubQuery. It should not be a clause condition (such as Where or Having), since those are not meant to be composed.
- **Throws:**
  - `java.lang.IllegalArgumentException` — if condition is null, or if condition is non-composable — a Criteria, a Clause condition (for example Where, Having, or OrderBy), an ON/USING connector, an ANY/ALL/SOME quantified-subquery operand, a standalone SubQuery, or a blank SqlExpression. An empty Junction is accepted and negates its Boolean identity (for example NOT (1 = 1))
...
- **Summary:** Creates a new NOT BETWEEN condition.
- **Parameters:**
  - `propName` (`String`) — the property/column name (must not be null, empty, or blank)
...
- **Parameters:**
  - `subQuery` (`SubQuery`) — the subquery to check for the absence of rows (must not be null)
- **Throws:**
  - `java.lang.IllegalArgumentException` — if subQuery is null
...
#### Public Fields
- **Declaration:** `public static final Operator EQUAL`
  - **Summary:** Equal operator (=).
  - **Nullability:** unspecified
- **Declaration:** `public static final Operator NOT_EQUAL`
  - **Summary:** Not-equal operator (!=).
  - **Nullability:** unspecified
- **Declaration:** `public static final Operator NOT_EQUAL_ANSI`
  - **Summary:** ANSI standard not equal operator (<>).
  - **Nullability:** unspecified
- **Declaration:** `public static final Operator NOT`
  - **Summary:** NOT operator.
  - **Nullability:** unspecified
- **Declaration:** `public static final Operator NOT_OP`
  - **Summary:** NOT operator symbol (!).
  - **Nullability:** unspecified
- **Declaration:** `public static final Operator XOR`
  - **Summary:** XOR (exclusive OR) operator.
  - **Nullability:** unspecified
- **Declaration:** `public static final Operator LIKE`
  - **Summary:** LIKE operator.
  - **Nullability:** unspecified
- **Declaration:** `public static final Operator NOT_LIKE`
  - **Summary:** NOT LIKE operator.
  - **Nullability:** unspecified
- **Declaration:** `public static final Operator AND`
  - **Summary:** AND operator.
  - **Nullability:** unspecified
- **Declaration:** `public static final Operator AND_OP`
  - **Summary:** AND operator symbol (&&).
  - **Nullability:** unspecified
- **Declaration:** `public static final Operator OR`
  - **Summary:** OR operator.
  - **Nullability:** unspecified
- **Declaration:** `public static final Operator OR_OP`
  - **Summary:** OR operator symbol (||).
  - **Nullability:** unspecified
- **Declaration:** `public static final Operator GREATER_THAN`
  - **Summary:** Greater than operator (>).
  - **Nullability:** unspecified
- **Declaration:** `public static final Operator GREATER_THAN_OR_EQUAL`
  - **Summary:** Greater than or equal operator (>=).
  - **Nullability:** unspecified
- **Declaration:** `public static final Operator LESS_THAN`
  - **Summary:** Less than operator (<).
  - **Nullability:** unspecified
- **Declaration:** `public static final Operator LESS_THAN_OR_EQUAL`
  - **Summary:** Less than or equal operator (<=).
  - **Nullability:** unspecified
- **Declaration:** `public static final Operator BETWEEN`
  - **Summary:** BETWEEN operator.
  - **Nullability:** unspecified
- **Declaration:** `public static final Operator NOT_BETWEEN`
  - **Summary:** NOT BETWEEN operator.
  - **Nullability:** unspecified
- **Declaration:** `public static final Operator IS`
  - **Summary:** IS operator.
  - **Nullability:** unspecified
- **Declaration:** `public static final Operator IS_NOT`
  - **Summary:** IS NOT operator.
  - **Nullability:** unspecified
- **Declaration:** `public static final Operator EXISTS`
  - **Summary:** EXISTS operator.
  - **Nullability:** unspecified
- **Declaration:** `public static final Operator NOT_EXISTS`
  - **Summary:** NOT EXISTS operator.
  - **Nullability:** unspecified
- **Declaration:** `public static final Operator IN`
  - **Summary:** IN operator.
  - **Nullability:** unspecified
- **Declaration:** `public static final Operator NOT_IN`
  - **Summary:** NOT IN operator.
  - **Nullability:** unspecified
- **Declaration:** `public static final Operator ANY`
  - **Summary:** ANY operator.
  - **Nullability:** unspecified
- **Declaration:** `public static final Operator SOME`
  - **Summary:** SOME operator.
  - **Nullability:** unspecified
- **Declaration:** `public static final Operator ALL`
  - **Summary:** ALL operator.
  - **Nullability:** unspecified
- **Declaration:** `public static final Operator ON`
  - **Summary:** ON operator.
  - **Nullability:** unspecified
- **Declaration:** `public static final Operator USING`
  - **Summary:** USING operator.
  - **Nullability:** unspecified
- **Declaration:** `public static final Operator JOIN`
  - **Summary:** Basic JOIN operator.
  - **Nullability:** unspecified
- **Declaration:** `public static final Operator LEFT_JOIN`
  - **Summary:** LEFT JOIN operator (LEFT OUTER JOIN).
  - **Nullability:** unspecified
- **Declaration:** `public static final Operator RIGHT_JOIN`
  - **Summary:** RIGHT JOIN operator (RIGHT OUTER JOIN).
  - **Nullability:** unspecified
- **Declaration:** `public static final Operator FULL_JOIN`
  - **Summary:** FULL JOIN operator (FULL OUTER JOIN).
  - **Nullability:** unspecified
- **Declaration:** `public static final Operator CROSS_JOIN`
  - **Summary:** CROSS JOIN operator.
  - **Nullability:** unspecified
- **Declaration:** `public static final Operator INNER_JOIN`
  - **Summary:** INNER JOIN operator.
  - **Nullability:** unspecified
- **Declaration:** `public static final Operator NATURAL_JOIN`
  - **Summary:** NATURAL JOIN operator.
  - **Nullability:** unspecified
- **Declaration:** `public static final Operator WHERE`
  - **Summary:** WHERE clause operator.
  - **Nullability:** unspecified
- **Declaration:** `public static final Operator HAVING`
  - **Summary:** HAVING clause operator.
  - **Nullability:** unspecified
- **Declaration:** `public static final Operator GROUP_BY`
  - **Summary:** GROUP BY clause operator.
  - **Nullability:** unspecified
- **Declaration:** `public static final Operator ORDER_BY`
  - **Summary:** ORDER BY clause operator.
  - **Nullability:** unspecified
- **Declaration:** `public static final Operator LIMIT`
  - **Summary:** LIMIT clause operator.
  - **Nullability:** unspecified
- **Declaration:** `public static final Operator OFFSET`
  - **Summary:** OFFSET clause operator.
  - **Nullability:** unspecified
- **Declaration:** `public static final Operator FOR_UPDATE`
  - **Summary:** FOR UPDATE clause operator.
  - **Nullability:** unspecified
- **Declaration:** `public static final Operator UNION`
  - **Summary:** UNION operator.
  - **Nullability:** unspecified
- **Declaration:** `public static final Operator UNION_ALL`
  - **Summary:** UNION ALL operator.
  - **Nullability:** unspecified
- **Declaration:** `public static final Operator INTERSECT`
  - **Summary:** INTERSECT operator.
  - **Nullability:** unspecified
- **Declaration:** `public static final Operator EXCEPT`
  - **Summary:** EXCEPT operator.
  - **Nullability:** unspecified
- **Declaration:** `public static final Operator MINUS`
  - **Summary:** MINUS operator.
  - **Nullability:** unspecified
- **Declaration:** `public static final Operator EMPTY`
  - **Summary:** Empty operator (renders as an empty string).
  - **Nullability:** unspecified
...
#### Public Constructors
- **Signature:** `@Deprecated public RightJoin(final String joinEntity)`
- **Summary:** Creates a RIGHT JOIN clause for the specified table or entity without a join condition.
- **Deprecated:** always throws IllegalArgumentException because a qualified join requires an ON/USING predicate; use #RightJoin(String, Condition) instead, or CrossJoin for an unconditional join
- **Parameters:**
  - `joinEntity` (`String`) — the table or entity to join with. Can include alias (e.g., "orders o").
- **Throws:**
...
#### Public Constructors
- **Signature:** `public SqlExpression(final String literal)`
- **Summary:** Constructs a new SqlExpression with the specified raw SQL expression text.
- **Parameters:**
...
- **Summary:** Creates a SqlExpression instance for the given expression text.
- **Parameters:**
  - `literal` (`String`) — the raw SQL expression text (must not be null)
- **Returns:** a new SqlExpression instance for the given text
- **Throws:**
  - `java.lang.IllegalArgumentException` — if literal is null
- **Examples:**
  - ```java
...
- **Summary:** Creates an equality expression between the given expression and a value.
- **Contract:**
...
- **Throws:**
  - `java.lang.IllegalArgumentException` — if expr is null, empty, or blank, or if value is a Float or Double that is NaN or infinite
- **Examples:**
  - ```java
...
- **Summary:** Creates a not-equal expression between the given expression and a value.
- **Contract:**
...
- **Throws:**
  - `java.lang.IllegalArgumentException` — if expr is null, empty, or blank, or if minValue or maxValue is a Float or Double that is NaN or infinite
- **Examples:**
  - ```java
...
- **Summary:** Creates a NOT BETWEEN expression for the given expression with min and max values.
- **Contract:**
  - A value satisfies NOT BETWEEN min AND max when it is strictly less than min or strictly greater than max.
- **Parameters:**
  - `expr` (`String`) — the expression to test
...
- **Summary:** Creates an IS NULL expression for the specified expression.
- **Parameters:**
...
- **Summary:** Creates an IS NOT NULL expression for the specified expression.
- **Parameters:**
...
- **Summary:** Creates a SQL expression that tests whether the specified string expression is either NULL or the empty string.
- **Parameters:**
  - `expr` (`String`) — the column reference or expression to check
- **Returns:** a parenthesized IS NULL OR = '' expression
- **Throws:**
  - `java.lang.IllegalArgumentException` — if expr is null, empty, or blank
- **Examples:**
  - ```java
...
- **Summary:** Creates a SQL expression that tests whether the specified string expression is both non-NULL and different from the empty string.
- **Parameters:**
  - `expr` (`String`) — the column reference or expression to check
- **Returns:** a parenthesized IS NOT NULL AND <> '' expression
- **Throws:**
  - `java.lang.IllegalArgumentException` — if expr is null, empty, or blank
- **Examples:**
  - ```java
...
- **Summary:** Creates an AND expression combining multiple expressions.
- **Contract:**
...
- **Throws:**
  - `java.lang.IllegalArgumentException` — if an element is null, empty, or blank
- **Examples:**
  - ```java
...
- **Summary:** Creates an OR expression combining multiple expressions.
- **Contract:**
...
- **Summary:** Creates a subtraction expression for the given objects.
- **Deprecated:** Use #subtract(Object...) instead to avoid confusion with the SQL MINUS set operation.
- **Parameters:**
  - `operands` (`Object[]`) — the values to subtract; a null or empty array yields an empty string
- **Returns:** a SQL representation of the subtraction expression, or an empty string if no operands are supplied
- **Throws:**
...
- **Throws:**
  - `java.lang.IllegalArgumentException` — if base or value is null, empty, or blank
- **Examples:**
  - ```java
...
- **Throws:**
  - `java.lang.IllegalArgumentException` — if dividend or divisor is null, empty, or blank
- **Examples:**
  - ```java
...
- **Throws:**
  - `java.lang.IllegalArgumentException` — if base or exponent is null, empty, or blank
- **Examples:**
  - ```java
...
- **Returns:** a CONCAT function string of the form CONCAT(expr1, expr2)
- **Throws:**
  - `java.lang.IllegalArgumentException` — if expr1 or expr2 is null, empty, or blank
- **Examples:**
  - ```java
...
- **Throws:**
  - `java.lang.IllegalArgumentException` — if expr, oldString, or replacement is null, empty, or blank
- **Examples:**
  - ```java
...
- **Throws:**
  - `java.lang.IllegalArgumentException` — if expr or padExpr is null, empty, or blank
- **Examples:**
  - ```java
...
- **Parameters:**
  - (none)
- **Returns:** the hash code of the literal, or 0 if the literal is null (only possible for an uninitialized instance produced by the package-private default constructor, e.g., during Kryo deserialization)
- **Examples:**
  - ```java
...
- **Parameters:**
  - `obj` (`Object`) — the object to compare with
- **Returns:** true if the objects have the same runtime class and literal string
- **Examples:**
  - ```java
...
- **Throws:**
  - `java.lang.IllegalArgumentException` — if sql is null, empty, or blank; contains a named (:name) or MyBatis #{...} placeholder; or contains a positional placeholder without a corresponding binding
...
- **Throws:**
  - `java.lang.IllegalArgumentException` — if sql is null, empty, or blank; parameters is null; the SQL contains a named (:name) or MyBatis #{...} placeholder; the number of positional placeholders differs from the number of bindings; or an object-array binding contains a cycle
...
- **Parameters:**
  - `entityName` (`String`) — the entity/table name; may be null or empty, in which case it is stored as the empty string
  - `sql` (`String`) — complete raw query-expression text (must not be null, empty, or blank)
- **Throws:**
  - `java.lang.IllegalArgumentException` — if sql is null, empty, or blank; contains a named (:name) or MyBatis #{...} placeholder; or contains a positional placeholder without a corresponding binding
...
- **Summary:** Creates a structured single-property subquery for an entity name.
- **Parameters:**
  - `entityName` (`String`) — the entity/table name (must not be null, empty, or blank); a bare, optionally schema-qualified name. An inline alias such as users u is not recognized by #toSql(NamingPolicy), which converts the whole text as one identifier; build aliased sub-queries with a query builder instead
  - `propName` (`String`) — the property to select (must not be null, empty, or blank)
  - `condition` (`Condition`) — an optional trailing condition, clause, or Criteria. A predicate is wrapped in Where; null, a blank expression, or an empty Criteria adds no clause. An empty Junction is preserved as its true/false Boolean identity.
- **Throws:**
  - `java.lang.IllegalArgumentException` — if entityName or propName is null, empty, or blank, if condition uses an ON/USING operator, is not a complete predicate (for example a standalone SubQuery or quantified operand), or is a Criteria that carries a SELECT modifier (e.g. DISTINCT) — none of which are valid here
...
- **Parameters:**
  - `entityName` (`String`) — the entity/table name (must not be null, empty, or blank); a bare, optionally schema-qualified name. An inline alias such as users u is not recognized by #toSql(NamingPolicy), which converts the whole text as one identifier; build aliased sub-queries with a query builder instead
  - `propNames` (`Collection<String>`) — collection of property names to select (must not be null or empty and must not contain null, empty, or blank names)
  - `condition` (`Condition`) — an optional trailing condition, clause, or Criteria. A predicate is wrapped in Where; null, a blank expression, or an empty Criteria adds no clause. An empty Junction is preserved as its true/false Boolean identity.
- **Throws:**
  - `java.lang.IllegalArgumentException` — if entityName is null, empty, or blank, if propNames is null or empty, if propNames contains null, empty, or blank names, if condition uses an ON/USING operator, is not a complete predicate (for example a standalone SubQuery or quantified operand), or is a Criteria that carries a SELECT modifier (e.g. DISTINCT) — none of which are valid here
...
- **Parameters:**
  ...
  - `condition` (`Condition`) — an optional trailing condition, clause, or Criteria. A predicate is wrapped in Where; null, a blank expression, or an empty Criteria adds no clause. An empty Junction is preserved as its true/false Boolean identity.
- **Throws:**
  - `java.lang.IllegalArgumentException` — if entityClass is null, if propName is null, empty, or blank, if condition uses an ON/USING operator, is not a complete predicate (for example a standalone SubQuery or quantified operand), or is a Criteria that carries a SELECT modifier (e.g. DISTINCT) — none of which are valid here
...
- **Parameters:**
  - (none)
- **Returns:** the entity/table name; an empty string for an initialized unnamed raw or builder-backed subquery, or null only for an uninitialized serialization-framework instance
- **Examples:**
  - ```java
...
- **Parameters:**
  - (none)
- **Returns:** the entity class, or null if created with an entity name string, raw SQL, or a builder snapshot
- **Examples:**
  - ```java
...
- **Parameters:**
  - (none)
- **Returns:** immutable list of property names to select, or null for raw SQL and builder-backed subqueries
- **Examples:**
  - ```java
...
- **Parameters:**
  - (none)
- **Returns:** the normalized condition/clause, or null if none or for a raw SQL or builder-backed subquery
- **Examples:**
  - ```java
...
##### parameters(...) -> ImmutableList<Object>
- **Signature:** `@Override public ImmutableList<Object> parameters()`
- **Summary:** Returns this subquery's parameter values.
- **Contract:**
  - Raw bindings were snapshotted at construction (see #SubQuery(String, java.util.Collection)); when any of them is an array, Date, or Calendar, a fresh list holding fresh defensive copies of those bindings is built on every call, so mutating a returned element never affects this subquery or a later call.
- **Parameters:**
  - (none)
- **Returns:** an immutable list of parameter values (array/Date/Calendar raw bindings are defensive copies), or an empty immutable list when none were captured
- **Examples:**
  - ```java
...
- **Contract:**
  - Render the subquery through a SqlBuilder when condition identifiers must also resolve against entity column mappings.
- **Parameters:**
...
- **Contract:**
  - Captured raw bindings are hashed by content (array bindings element-wise via N#deepHashCode(Object)), consistent with #equals(Object); because array/Date/Calendar bindings were snapshotted at construction, the hash is stable even if the caller later mutates the originals.
- **Parameters:**
  - (none)
- **Returns:** hash code based on SQL, entity name/class, properties, condition, and captured raw bindings
- **Examples:**
  - ```java
...
- **Contract:**
  - Two subqueries are equal only when they have the exact same runtime class and all of their identity fields are equal: the entity name, the entity class, the selected properties, the stored SQL string, the condition, and captured raw bindings.
  - Raw bindings are compared by content (array bindings element-wise via N#deepEquals(Object, Object), other bindings through their own equals); since array/Date/Calendar bindings are snapshotted at construction, the result does not change when the caller later mutates the originals.
...
- **Parameters:**
  - `obj` (`Object`) — the object to compare with
- **Returns:** true if the objects have the same runtime class and identity fields, false otherwise
- **Examples:**
  - ```java
    // Same raw SQL: equal
    SubQuery a = Filters.subQuery("SELECT id FROM users");
...
#### Public Constructors
- **Signature:** `@Beta public Using(final String... columnNames)`
