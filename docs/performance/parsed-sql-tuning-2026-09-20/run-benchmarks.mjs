...
const mode=process.argv[6]??'original';
...
for(const [version,fork] of runs){
 const library=version==='current'?join(work,'final.jar'):join(old,version==='before'?'current.jar':'baseline.jar');
 const classpath=[join(work,'harness'),library,deps[version==='baseline'?'baseline':'current']].join(process.platform==='win32'?';':':');
 const started=new Date().toISOString();
 console.log('Starting '+version+' fork '+fork+' at '+started);
 const result=await new Promise((accept,reject)=>{
  const p=spawn(java,['-Xms512m','-Xmx1024m','-XX:+UseG1GC','-cp',classpath,'ParserBenchmark',version,String(fork),mode],{windowsHide:true});
