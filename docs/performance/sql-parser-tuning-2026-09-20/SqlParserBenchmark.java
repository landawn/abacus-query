...
public final class SqlParserBenchmark {
 ...
 private static List<Case> cases(){
  ...
  for(int op:new int[]{0,2,3})for(String value:new String[]{"#{id}","#>"})c.add(new Case((op==0?"tokenize_":op==2?"next_":"end_")+(value.startsWith("#{")?"mybatis_single":"hash_operator"),op,value,"",0,false,0));
  for(int size:new int[]{256,1024,4096,8192})c.add(new Case("tokenize_dense_concat_"+size,0,"SELECT "+"?||".repeat(size-1)+"? FROM t","",0,false,0));
 ...
 public static void main(String[] args)throws Exception{
  ...
  for(int i=0;i<CASES.size();i++){Case c=CASES.get(i);for(int k=0;k<32;k++)inputs[i][k]=new String(c.sql.toCharArray());int n=1;Sample s=sample(c,inputs[i],n);while(s.ns<2_000_000&&n<65536){n*=2;s=sample(c,inputs[i],n);}if(steady){long warmEnd=System.nanoTime()+250_000_000L,warmed=0;do{s=sample(c,inputs[i],n);warmed+=n;}while(System.nanoTime()<warmEnd||(c.op==13&&warmed<20000));}else{for(int w=0;w<4;w++)s=sample(c,inputs[i],n);}operations[i]=Math.max(1,Math.min(262144,(int)Math.ceil((steady?20_000_000.0:5_000_000.0)*n/Math.max(1,s.ns))));}
