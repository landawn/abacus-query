// Run: node docs/performance/sql-parser-tuning-2026-09-20/summarize.cjs
...
for(const name of fs.readdirSync(dir).filter(x=>/^(baseline|current|before|baseline-control)-\d+\.csv$/.test(x))){
 ...
 for(const row of rows){if(row.length!==8)throw Error('Bad row '+name);const [version,fork,workload,round,ops,ns,bytes,same]=row;
  ...
  let f=g.forks.get(fork);if(!f)g.forks.set(fork,f={times:[],bytes:[],rounds:new Set()});if(f.rounds.has(round))throw Error('Duplicate round');f.rounds.add(round);f.times.push(+ns);f.bytes.push(+bytes);samples++;
 }
}
const summary=[],forks=[];
for(const g of groups.values()){
 const f=[...g.forks].map(([fork,v])=>({version:g.version,case:g.workload,fork:Number(fork),ns:median(v.times),bytes:median(v.bytes)}));
...
fs.writeFileSync(path.join(dir,'per-fork.csv'),csv(forks,['version','case','fork','ns','bytes']));
...
const result={samples,cases,latencyWins:cases.filter(x=>x.speedup>1).length,allocationLower:cases.filter(x=>x.allocation_change<0).length,allocationEqual:cases.filter(x=>x.allocation_change===0).length,allocationHigher:cases.filter(x=>x.allocation_change>0).length};
