...
public class ExceptTest extends TestBase {
    ...
    public void testGetParameters() {
        SubQuery subQuery = ConditionFactory.subQuery("orders", Arrays.asList("customer_id"),
                ConditionFactory.and(ConditionFactory.lt("order_date", "2023-01-01"), ConditionFactory.eq("status", "cancelled")));
        Except except = ConditionFactory.except(subQuery);
    ...
    public void testClearParameters() {
        SubQuery subQuery = ConditionFactory.subQuery("products", Arrays.asList("id"),
    ...
    public void testComplexExceptScenarios() {
        ...
        SubQuery customersWithOrders = ConditionFactory.subQuery("orders", Arrays.asList("DISTINCT customer_id"),
        ...
        SubQuery recentlySold = ConditionFactory.subQuery("order_items oi JOIN orders o ON oi.order_id = o.id", Arrays.asList("oi.product_id"),
        ...
        SubQuery managers = ConditionFactory.subQuery("employees", Arrays.asList("employee_id"), ConditionFactory.or(ConditionFactory.eq("is_manager", true),
                ConditionFactory.like("title", "%Manager%"), ConditionFactory.like("title", "%Director%")));
    ...
    public void testExceptWithCriteria() {
        ...
        Criteria criteria = ConditionFactory.criteria().where(ConditionFactory.eq("status", "active")).except(excludedUsers);
    ...
    public void testExceptWithComplexSubQuery() {
        ...
        SubQuery complexSubQuery = ConditionFactory.subQuery("users u JOIN departments d ON u.dept_id = d.id", Arrays.asList("u.id"),
                ConditionFactory.and(ConditionFactory.eq("d.location", "Remote"),
                        ConditionFactory.or(ConditionFactory.eq("u.type", "contractor"), ConditionFactory.lt("u.hire_date", "2020-01-01")),
