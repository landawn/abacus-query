...
public class ExistsTest extends TestBase {
    ...
    public void testGetParameters() {
        SubQuery subQuery = ConditionFactory.subQuery("orders", 
            ...
            ConditionFactory.and(
                ConditionFactory.eq("customer_id", 123),
                ConditionFactory.eq("status", "pending")
            )
        );
        Exists exists = ConditionFactory.exists(subQuery);
    ...
    public void testClearParameters() {
        SubQuery subQuery = ConditionFactory.subQuery("products", 
            Arrays.asList("id"), 
            ConditionFactory.in("category_id", Arrays.asList(1, 2, 3))
        );
        Exists exists = ConditionFactory.exists(subQuery);
    ...
    public void testComplexExistsScenarios() {
        ...
        SubQuery hasOrders = ConditionFactory.subQuery(
            "orders",
            Arrays.asList("1"),
            ConditionFactory.and(
                ConditionFactory.eq("orders.customer_id", "customers.id"),
        ...
        SubQuery hasStock = ConditionFactory.subQuery(
            "inventory",
            Arrays.asList("1"),
            ConditionFactory.and(
                ConditionFactory.eq("inventory.product_id", "products.id"),
                ConditionFactory.gt("quantity", 0)
            )
        );
