...
public class UnionAllTest extends TestBase {
    ...
    public void testWithComplexSubQuery() {
        SubQuery subQuery = CF.subQuery("orders", 
            Arrays.asList("id", "customer_id", "total"), 
            CF.and(CF.eq("region", "WEST"), CF.gt("date", "2024-01-01")));
        UnionAll unionAll = CF.unionAll(subQuery);
    ...
    public void testGetParameters() {
        SubQuery subQuery = CF.subQuery("sales", 
            Arrays.asList("product_id", "quantity"), 
            CF.eq("store_id", 42));
        UnionAll unionAll = CF.unionAll(subQuery);
    ...
    public void testClearParameters() {
        SubQuery subQuery = CF.subQuery("logs", 
            Arrays.asList("timestamp", "message"), 
            CF.between("timestamp", "2024-01-01", "2024-12-31"));
        UnionAll unionAll = CF.unionAll(subQuery);
    ...
    public void testCopy() {
        SubQuery subQuery = CF.subQuery("transactions", 
            Arrays.asList("id", "amount"), 
            CF.eq("status", "completed"));
    ...
    public void testPracticalExample2() {
        ...
        SubQuery activeUsers = CF.subQuery("active_users", 
    ...
    public void testPracticalExample3() {
        ...
        SubQuery eastRegion = CF.subQuery("orders", 
            Arrays.asList("*"), 
            CF.eq("region", "EAST"));
        SubQuery westRegion = CF.subQuery("orders", 
            Arrays.asList("*"), 
            CF.eq("region", "WEST"));
