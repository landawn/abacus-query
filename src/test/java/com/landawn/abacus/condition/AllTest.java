...
public class AllTest extends TestBase {
    ...
    public void testGetParameters() {
        SubQuery subQuery = ConditionFactory.subQuery("products", 
            ...
            ConditionFactory.and(
                ConditionFactory.eq("category", "Premium"),
    ...
    public void testClearParameters() {
        SubQuery subQuery = ConditionFactory.subQuery("grades", 
            Arrays.asList("score"), 
            ConditionFactory.between("year", 2020, 2023)
        );
        All all = ConditionFactory.all(subQuery);
        ...
        Assertions.assertTrue(all.getParameters().isEmpty());
    }
    ...
    public void testComplexAllScenarios() {
        ...
        SubQuery competitorPrices = ConditionFactory.subQuery("competitors", 
            Arrays.asList("price"), 
            ConditionFactory.and(
                ConditionFactory.eq("region", "Europe"),
                ConditionFactory.eq("product_type", "Premium"),
                ConditionFactory.eq("active", true)
            )
        );
        ...
        SubQuery managerSalaries = ConditionFactory.subQuery(
            "employees",
            Arrays.asList("salary"),
            ConditionFactory.and(
                ConditionFactory.eq("is_manager", true),
                ConditionFactory.eq("department_id", Expression.of("e.department_id"))
            )
        );
